...
local CommandData = {
	...
	},
	{
		Category = "MOVEMENT",
		Display = "fly [speed]",
		Aliases = { "fly", "cfly", "cframefly" },
		Description = "Enables CFrame-based flight. Hold WASD to move. Optionally pass a speed multiplier, e.g. fly 2. Camera direction controls where you fly.",
		Undo = "unfly / uncfly",
	},
	{
		Category = "MOVEMENT",
		Display = "unfly",
		Aliases = { "unfly", "uncfly", "uncframefly" },
		Description = "Disables flight and returns you to normal movement.",
	},
	{
		Category = "MOVEMENT",
		Display = "bypassfly [speed]",
		Aliases = { "bypassfly", "bfly" },
		Description = "Flight mode that detaches your character from workspace to help bypass anti-cheat. When disabled, you will teleport back to the flight part position. Disable normal fly first.",
		Undo = "unbypassfly / unbfly",
	},
	{
		Category = "MOVEMENT",
		Display = "unbypassfly",
		Aliases = { "unbypassfly", "unbfly" },
		Description = "Disables bypass flight and restores your character back into the workspace.",
	},
	{
		Category = "MOVEMENT",
		Display = "loopwalkspeed [speed]",
		Aliases = { "loopwalkspeed", "loopws" },
		Description = "Sets your WalkSpeed every frame so the server cannot revert it. Pass a number like loopwalkspeed 50. Defaults to your current WalkSpeed if no value is given.",
		Undo = "unloopwalkspeed / unloopws",
	},
	{
		Category = "MOVEMENT",
		Display = "unloopwalkspeed",
		Aliases = { "unloopwalkspeed", "unloopws" },
		Description = "Stops the WalkSpeed loop and restores your original WalkSpeed.",
	},
	{
		Category = "MOVEMENT",
		Display = "cframespeed [speed]",
		Aliases = { "cframespeed", "cfspeed", "cfws" },
		Description = "Moves your character via CFrame offsets while you walk, making you faster without touching WalkSpeed. Default speed multiplier is 2. Use WASD to move.",
		Undo = "uncframespeed / uncfspeed",
	},
	{
		Category = "MOVEMENT",
		Display = "uncframespeed",
		Aliases = { "uncframespeed", "uncfspeed", "uncfws" },
		Description = "Disables CFrame speed boost.",
	},
	{
		Category = "COMBAT",
		Display = "fling <player>",
		Aliases = { "fling" },
		Description = "Flings a target player by teleporting to them and applying extreme velocity to their character. The player name is required.",
	},
}
...
local function cleanup()
	...
	-- Call any cleanup registered by ported commands (flight parts, character restore, etc.)
	if type(Runtime.PortedCleanup) == "function" then
		pcall(Runtime.PortedCleanup)
	end
...
-- ============================================================
-- PORTED FROM OPADMIN: Helper Functions
-- ============================================================

-- State table for ported commands
local CmdState = {
	fly = { enabled = false, part = nil, speed = 1 },
	bfly = { enabled = false, part = nil, speed = 1, oldSubject = nil, oldSpeed = 16, oldParent = nil },
	loopws = { enabled = false, oldSpeed = 16 },
	cframespeed = { enabled = false, speed = 2 },
	fling = {},
}

-- Named connection registry for ported commands. Uses Runtime.Connections so
-- the existing cleanup() + disconnectAll() path handles teardown on re-execute.
local PortedConnectionKeys = {}
local function addPortedConnection(name, connection)
	-- Disconnect any previous connection with the same name
	if PortedConnectionKeys[name] then
		local idx = PortedConnectionKeys[name]
		if Runtime.Connections[idx] then
			pcall(function() Runtime.Connections[idx]:Disconnect() end)
			Runtime.Connections[idx] = nil
		end
	end
	-- Register into Runtime.Connections so cleanup() teardown covers it
	Runtime.Connections[#Runtime.Connections + 1] = connection
	PortedConnectionKeys[name] = #Runtime.Connections
end

local function removePortedConnection(name)
	if not PortedConnectionKeys[name] then return end
	local idx = PortedConnectionKeys[name]
	if Runtime.Connections[idx] then
		pcall(function() Runtime.Connections[idx]:Disconnect() end)
		Runtime.Connections[idx] = nil
	end
	PortedConnectionKeys[name] = nil
end

-- Hook into cleanup() so re-execute destroys flight parts and restores character
Runtime.PortedCleanup = function()
	if CmdState.fly.part then
		pcall(function() CmdState.fly.part:Destroy() end)
		CmdState.fly.part = nil
	end
	CmdState.fly.enabled = false

	if CmdState.bfly.part then
		pcall(function() CmdState.bfly.part:Destroy() end)
		CmdState.bfly.part = nil
	end
	CmdState.bfly.enabled = false
	-- Restore character parent if it was detached by bypassfly
	local char = LocalPlayer.Character
	if char and CmdState.bfly.oldParent and not char.Parent then
		pcall(function() char.Parent = CmdState.bfly.oldParent end)
	end
	-- Restore camera subject
	local cam = Workspace.CurrentCamera
	if cam and CmdState.bfly.oldSubject then
		pcall(function() cam.CameraSubject = CmdState.bfly.oldSubject end)
	end

	CmdState.loopws.enabled = false
	CmdState.cframespeed.enabled = false
end
...
-- Reads WASD input and returns a local-space direction vector
local function getMoveVector(speed)
	speed = speed or 1
	...
	if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) or UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) then direction = direction + Vector3.new(0, -1, 0) end
	return direction * speed
end

-- ============================================================
-- PORTED FROM OPADMIN: fly / unfly
-- ============================================================

local function enableFly(text)
	local state = CmdState.fly
	if state.enabled then return end

	local _, args = extractCommandArgs(text or "fly")
	local speed = tonumber(args:match("%-?%d+%.?%d*")) or 1
	...
	flightPart.CFrame = (getCharacter() and getCharacter():GetPivot()) or CFrame.new()
	flightPart.Parent = Workspace
	state.part = flightPart

	addPortedConnection("fly_heartbeat", RunService.Heartbeat:Connect(function()
		pcall(function()
			if not state.enabled or not flightPart or not flightPart.Parent then return end
			local char = getCharacter()
			if not char then return end

			local camCF = Workspace.CurrentCamera.CFrame
			local oldPos = flightPart.Position
			flightPart.CFrame = CFrame.lookAt(oldPos, camCF * CFrame.new(0, 0, -250).Position)

			local humanoid = char:FindFirstChildOfClass("Humanoid")
			if humanoid then
				pcall(function() humanoid:ChangeState(Enum.HumanoidStateType.Running) end)
			end
			char:PivotTo(flightPart.CFrame)

			local hrp = char:FindFirstChild("HumanoidRootPart")
			if hrp then hrp.AssemblyLinearVelocity = Vector3.zero end

			local offset = getMoveVector(state.speed)
			flightPart.CFrame = flightPart.CFrame * CFrame.new(offset)
		end)
	end))
end
...
local function disableFly()
	...
	state.enabled = false
	removePortedConnection("fly_heartbeat")

	if state.part then
		pcall(function() state.part:Destroy() end)
		state.part = nil
	end
end

-- ============================================================
-- PORTED FROM OPADMIN: bypassfly / unbypassfly
-- ============================================================

local function enableBypassFly(text)
	local state = CmdState.bfly
	if CmdState.fly.enabled then return end
	if state.enabled then return end

	local _, args = extractCommandArgs(text or "bypassfly")
	local speed = tonumber(args:match("%-?%d+%.?%d*")) or 1
	...
	local cam = Workspace.CurrentCamera
	state.oldSubject = cam.CameraSubject

	local humanoid = getHumanoid()
	state.oldSpeed = humanoid and humanoid.WalkSpeed or 16

	local char = getCharacter()
	state.oldParent = char and char.Parent

	local flightPart = Instance.new("Part")
	flightPart.Anchored = true
	flightPart.Transparency = 0.5
	flightPart.CanCollide = false
	flightPart.CFrame = char and char:GetPivot() or CFrame.new()
	flightPart.Parent = Workspace
	state.part = flightPart

	-- Detach character from workspace (bypass trick)
	if char then
		pcall(function() char.Parent = nil end)
	end
	pcall(function() cam.CameraSubject = flightPart end)

	addPortedConnection("bfly_heartbeat", RunService.Heartbeat:Connect(function()
		pcall(function()
			if not state.enabled or not flightPart or not flightPart.Parent then return end
...
local function disableBypassFly()
	local state = CmdState.bfly
	if not state.enabled then return end

	state.enabled = false
	removePortedConnection("bfly_heartbeat")

	local char = getCharacter()
	if state.part then
		if char then
			pcall(function()
				char.Parent = state.oldParent or Workspace
				char:PivotTo(state.part.CFrame)
			end)
		end
	...
	pcall(function() cam.CameraSubject = state.oldSubject or getHumanoid() end)

	local humanoid = getHumanoid()
	if humanoid then
		pcall(function() humanoid.WalkSpeed = state.oldSpeed end)
	end
end

-- ============================================================
-- PORTED FROM OPADMIN: loopwalkspeed / unloopwalkspeed
-- ============================================================

local function enableLoopWalkspeed(text)
	local state = CmdState.loopws
	local _, args = extractCommandArgs(text or "loopwalkspeed")
	local speed = tonumber(args:match("%-?%d+%.?%d*"))
	...
	state.enabled = true
	state.oldSpeed = defaultSpeed

	addPortedConnection("loopws_heartbeat", RunService.Heartbeat:Connect(function()
		pcall(function()
			local hum = getHumanoid()
...
local function disableLoopWalkspeed()
	...
	state.enabled = false
	removePortedConnection("loopws_heartbeat")
...
-- ============================================================
-- PORTED FROM OPADMIN: cframespeed / uncframespeed
-- ============================================================

local function enableCFrameSpeed(text)
	local state = CmdState.cframespeed
	local _, args = extractCommandArgs(text or "cframespeed")
	local speed = tonumber(args:match("%-?%d+%.?%d*")) or 2

	state.enabled = true
	state.speed = speed

	addPortedConnection("cframespeed_heartbeat", RunService.Heartbeat:Connect(function()
		pcall(function()
			local char = getCharacter()
			if not char then return end
			local humanoid = char:FindFirstChildOfClass("Humanoid")
			...
			if moveDir.Magnitude > 0 then
				local hrp = char:FindFirstChild("HumanoidRootPart")
				if hrp then
					hrp.CFrame = hrp.CFrame + moveDir * state.speed
				end
			end
		end)
	end))
...
local function disableCFrameSpeed()
	...
	state.enabled = false
	removePortedConnection("cframespeed_heartbeat")
end

-- ============================================================
-- PORTED FROM OPADMIN: fling
-- ============================================================

local function flingPlayer(text)
	local _, args = extractCommandArgs(text or "")
	local targetName = args:match("^(%S+)") or ""
	if targetName == "" then return end

	local targetPlayer = nil
	for _, plr in ipairs(Services.Players:GetPlayers()) do
		if plr ~= LocalPlayer and string.lower(plr.Name):find(string.lower(targetName), 1, true) then
			targetPlayer = plr
			break
		end
	end
	...
	local maxVel = Vector3.new(2500, 2500, 2500)
	local zeroVel = Vector3.zero
	local startTick = tick()

	removePortedConnection("fling_heartbeat")
	addPortedConnection("fling_heartbeat", RunService.Heartbeat:Connect(function()
		pcall(function()
			local targetVel = targetHRP.AssemblyLinearVelocity
			if targetVel.Magnitude < 100 and tick() - startTick < 5 then
				local futureSpeed = (targetHumanoid.WalkSpeed * 68.75) / 100
				local predictedCF = targetHRP.CFrame + targetHumanoid.MoveDirection * futureSpeed
				hrp.CFrame = predictedCF
				hrp.AssemblyLinearVelocity = maxVel
			else
				hrp.AssemblyLinearVelocity = zeroVel
				pcall(function() LocalPlayer.Character:PivotTo(oldPos) end)
				removePortedConnection("fling_heartbeat")
			end
		end)
	end))
...
local function registerCommands()
	Runtime.Commands = {
		...
		unfly = disableFly,
		uncfly = disableFly,
		uncframefly = disableFly,

		-- PORTED: bypassfly
		bypassfly = enableBypassFly,
		bfly = enableBypassFly,
		unbypassfly = disableBypassFly,
		unbfly = disableBypassFly,
		disablebfly = disableBypassFly,
		stopbfly = disableBypassFly,

		-- PORTED: loopwalkspeed
		loopwalkspeed = enableLoopWalkspeed,
		loopws = enableLoopWalkspeed,
		unloopwalkspeed = disableLoopWalkspeed,
		unloopws = disableLoopWalkspeed,

		-- PORTED: cframespeed
		cframespeed = enableCFrameSpeed,
		cfspeed = enableCFrameSpeed,
		cfws = enableCFrameSpeed,
		uncframespeed = disableCFrameSpeed,
		uncfspeed = disableCFrameSpeed,
		uncfws = disableCFrameSpeed,
