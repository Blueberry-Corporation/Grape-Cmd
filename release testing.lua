...
local Settings = {
	...
	Themes = {
		--// GUI
		PrimaryBGColor = Color3.fromRGB(10, 12, 35), -- default: 25,25,25
		TopbarColor = Color3.fromRGB(15, 18, 50), -- default: 26,26,26
		...
		OutlineColor = Color3.fromRGB(0, 0, 0), -- border/shadow: default 0,0,0
		OutlineColor2 = Color3.fromRGB(60, 80, 200), -- outline of ui elements: default 45,45,45
		--// Command Bar
		CommandLinesColor = Color3.fromRGB(10, 12, 35),
		CommandCenterBarColor = Color3.fromRGB(10, 12, 35),
		EdgeColor = Color3.fromRGB(10, 12, 35),
		AutofillColor = Color3.fromRGB(10, 12, 35),
...
--// if re-executed, removes the old ui
if getgenv then
	if getgenv().BlueberryCmdLoaded then
		getgenv().BlueberryCmdInterfacePath.Parent = nil
...
if Checks.CheckIfFolderFuncs then
	if not isfolder("BlueberryCmd-V1") then
		makefolder("BlueberryCmd-V1")
	end

	if not isfolder("BlueberryCmd-V1/Plugins") then
		makefolder("BlueberryCmd-V1/Plugins")
	end

	if not isfolder("BlueberryCmd-V1/Waypoints") then
		makefolder("BlueberryCmd-V1/Waypoints")
	end

	if not isfolder("BlueberryCmd-V1/KeyBinds") then
		makefolder("BlueberryCmd-V1/KeyBinds")
	end
end
...
local CommandToggles = {
	...
	LoopFling = false,
	ESP = false,
	AlreadyStartedLogging = false, -- used for log commands
	...
	Glitch = false,
	LoopWalkspeed = false,
	LoopWalkspeedValue = 16,
	LoopJumpPower = false,
	LoopJumpPowerValue = 50,
	SitFly = false,
	SitFlyRSConn = nil,
	SitFlyCharConn = nil,
	SitFlySpeed = 50,
...
local NFuncs = {} --// BlueberryCmd Functions
local PlayerFuncs = {}
local UIFuncs = {
	--// Popup
	FrameSpeed = 0.5,
...
function PlayerFuncs.Walkfling(Power)
	CommandToggles.Walkfling = false --// stop previous walkfling loop if re-running
	wait(0.3)
...
-- Back button connected once globally to avoid stacking listeners
do
	local _cmdTabs = Interface.Commands.Tabs
	local _back    = Interface.Commands.Topbar.Back
	local _speed   = 0.5
	local _cmdCooldown = false

	_back.MouseButton1Click:Connect(function()
		if _back.Icon.Image == "rbxassetid://11422143469" then
			if not _cmdCooldown then
				_cmdCooldown = true
				_back.Icon.Image = "rbxassetid://11293977875"
				_cmdTabs.Commands.Visible = true
				Services.TweenService:Create(_cmdTabs.Commands, TweenInfo.new(_speed), {Position = UDim2.new(0,0,0,0)}):Play()
				Services.TweenService:Create(_cmdTabs.CommandInfoExample, TweenInfo.new(_speed), {Position = UDim2.new(0,310,0,0)}):Play()
				task.wait(_speed + 0.1)
				_cmdTabs.CommandInfoExample.Visible = false
				CurrentTab = "Commands"
				_cmdCooldown = false
			end
		end
...
function UIFuncs.AddCommand(Name, Description, Args)
	...
	NewCommand.MouseButton1Click:Connect(function()
		if not Cooldown then
			...
			Services.TweenService:Create(Tabs.Commands, TweenInfo.new(TweenSpeed), {Position = UDim2.new(0,-310,0,0)}):Play()
			Services.TweenService:Create(Tabs.CommandInfoExample, TweenInfo.new(TweenSpeed), {Position = UDim2.new(0,0,0,0)}):Play()
...
function UIFuncs.SendNotification(Title, Description, Duration, Style)
	-- Style: "success" | "warning" | "error" | nil
	spawn(
		function()
			...
			-- Icon based on style
			local iconId = nil
			local accentColor = Color3.fromRGB(60, 80, 200)
			if Style == "success" then
				iconId = "rbxassetid://15838874609"
				accentColor = Color3.fromRGB(80, 200, 120)
			elseif Style == "error" then
				iconId = "rbxassetid://3926305904"
				accentColor = Color3.fromRGB(220, 80, 80)
			elseif Style == "warning" then
				iconId = "rbxassetid://3926305532"
				accentColor = Color3.fromRGB(220, 160, 60)
			end

			-- Apply accent color to timer bar
			pcall(function() ClonedNotif.Timer.Timer.BackgroundColor3 = accentColor end)
			pcall(function() ClonedNotif.Outline.Color = accentColor end)
...
-- Gradient label for button
local BtnLabel = UIFuncs.CreateInstance(
	"TextLabel",
	{
		Parent = Button,
...
BtnGradient.Color = ColorSequence.new({
	...
	ColorSequenceKeypoint.new(0.4, Color3.fromRGB(20,  20,  80)),
...
spawn(
	function()
		...
		if Start:FindFirstChild("Title") then
			Start.Title.Text = "BlueberryCmd"
			local tg = Instance.new("UIGradient")
			tg.Color = ColorSequence.new({
				ColorSequenceKeypoint.new(0,   Color3.fromRGB(60,  130, 255)),
				ColorSequenceKeypoint.new(0.45, Color3.fromRGB(120, 60,  255)),
				ColorSequenceKeypoint.new(1,   Color3.fromRGB(180, 80,  255)),
			})
			tg.Rotation = 0
			tg.Parent = Start.Title
		end
		...
		UIFuncs.TypeWriter(Description, "BlueberryCmd is loading...")
	end
)
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification("BlueberryCmd", "Successfully unlocked every part in the game", 5)
	end
)
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification("BlueberryCmd", "Anti kick loaded", 5)
	end
)
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification("BlueberryCmd", "Anti teleport loaded", 5)
	end
)
...
Command.Add(
	...
	function()
		...
		if Tool.Parent ~= workspace then
			...
			UIFuncs.SendNotification("BlueberryCmd", "No tool grabber was found", 5)
		end
...
Command.Add(
	...
	function(Username)
		if not Services.SoundService.RespectFilteringEnabled then
			...
			UIFuncs.SendNotification('BlueberryCmd', 'Respect Filtering Enabled is on, so this command wont work.', 5)
		end
	end
)
...
Command.Add(
	...
	function(Property, Value)
		if Property and Value then
			...
			UIFuncs.SendNotification("BlueberryCmd", "Missing an argument", 5)
		end
	end
)
...
Command.Add(
	...
	function()
				...
				Plr.Chatted:Connect(
					function(Message)
						if CheckIfMessageHasBannedWord(Message) then
							...
							UIFuncs.SendNotification("BlueberryCmd", "Reported " .. Plr.Name .. " for " .. Reason, 5)
		...
		UIFuncs.SendNotification("BlueberryCmd", "Auto report has loaded!", 5)
	end
)
...
Command.Add(
	...
	function()
		...
		if fireclickdetector then
			...
			UIFuncs.SendNotification("BlueberryCmd", "Fired " .. tostring(Amount) .. " click detectors", 5)
		else
			UIFuncs.SendNotification("BlueberryCmd", "Your exploit doesnt support this command", 5)
		end
	end
)
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification("BlueberryCmd", "AntiFling turned on.", 3, "success")
		repeat
			task.wait()
...
Command.Add(
	...
	function()
		...
		if firetouchinterest then
			...
			UIFuncs.SendNotification("BlueberryCmd", "Fired " .. tostring(Amount) .. " touch interests", 5)
		else
			UIFuncs.SendNotification("BlueberryCmd", "Your exploit doesnt support this command", 5)
		end
	end
...
Command.Add(
	...
	function()
		...
		if fireproximityprompt then
			...
			UIFuncs.SendNotification("BlueberryCmd", "Fired " .. tostring(Amount) .. " proximity prompts", 5)
		else
			UIFuncs.SendNotification("BlueberryCmd", "Your exploit doesnt support this command", 5)
		end
	end
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification("BlueberryCmd", "Fired " .. tostring(Amount) .. " remotes", 5)
	end
)
...
Command.Add(
	...
	function(Gravity)
		...
		UIFuncs.SendNotification("BlueberryCmd", "Set gravity for unanchored parts to " .. tostring(Gravity), 5)
	end
)
...
Command.Add(
	{"Fly"},
	{"Fly [speed]", "Fly using SitFly method, character stays standing. ;fly 30 to set speed"},
	{"Speed (optional)"},
	function(Speed)
		local newSpeed = tonumber(Speed)
		-- if fly already active, just update speed
		if CommandToggles.SitFly then
			if newSpeed then
				CommandToggles.FlySpeed = newSpeed
				CommandToggles.SitFlySpeed = newSpeed
				UIFuncs.SendNotification("BlueberryCmd", "Fly speed set to " .. newSpeed, 3, "success")
			else
				UIFuncs.SendNotification("BlueberryCmd", "Fly is already on. Type ;unfly to stop.", 3, "warning")
			end
			return
		end
		if newSpeed then
		...
		CommandToggles.SitFly = true

		local controlModule = require(
			Services.LocalPlayer.PlayerScripts:WaitForChild("PlayerModule"):WaitForChild("ControlModule")
		)

		local function setupFly(root, char)
			if not root then return end
			for _, v in ipairs(root:GetChildren()) do
				if v.Name == "SFVelocity" or v.Name == "SFAlign" or v.Name == "SFAttachment" then v:Destroy() end
			end
			local att = Instance.new("Attachment"); att.Name = "SFAttachment"; att.Parent = root
			local lv = Instance.new("LinearVelocity"); lv.Name = "SFVelocity"; lv.Attachment0 = att
			lv.MaxForce = math.huge; lv.VectorVelocity = Vector3.new(0,0,0)
			lv.RelativeTo = Enum.ActuatorRelativeTo.World; lv.Parent = root
			local ao = Instance.new("AlignOrientation"); ao.Name = "SFAlign"; ao.Attachment0 = att
			ao.Mode = Enum.OrientationAlignmentMode.OneAttachment; ao.MaxTorque = math.huge
			ao.Responsiveness = 20; ao.Parent = root
			local hum = char:FindFirstChildWhichIsA("Humanoid")
			if hum then
				hum.PlatformStand = false
				hum.Sit = false -- standing, not sitting
			end
		end

		setupFly(Services.Character and Services.Character:FindFirstChild("HumanoidRootPart"), Services.Character)

		CommandToggles.SitFlyCharConn = Services.LocalPlayer.CharacterAdded:Connect(function(char)
			task.wait(0.15)
			if CommandToggles.SitFly then setupFly(char:FindFirstChild("HumanoidRootPart"), char) end
		end)

		CommandToggles.SitFlyRSConn = Services.RunService.RenderStepped:Connect(function()
			if not CommandToggles.SitFly then return end
			local root = Services.Character and Services.Character:FindFirstChild("HumanoidRootPart")
			if not root then return end
			local lv = root:FindFirstChild("SFVelocity")
			local ao = root:FindFirstChild("SFAlign")
			local hum = Services.Character:FindFirstChildWhichIsA("Humanoid")
			if lv and ao and hum then
				hum.Sit = false -- force standing
				ao.CFrame = Services.Camera.CFrame
				local dir = controlModule:GetMoveVector()
				local move = (Services.Camera.CFrame.RightVector * dir.X) + (Services.Camera.CFrame.LookVector * -dir.Z)
				...
				lv.VectorVelocity = move * CommandToggles.FlySpeed
			end
		end)

		UIFuncs.SendNotification("BlueberryCmd", "Fly on. Speed: " .. CommandToggles.FlySpeed .. ". Type ;unfly to stop.", 4, "success")
	end
)
...
Command.Add(
	...
	{"None"},
	function()
		CommandToggles.SitFly = false
		if CommandToggles.SitFlyRSConn then CommandToggles.SitFlyRSConn:Disconnect(); CommandToggles.SitFlyRSConn = nil end
		if CommandToggles.SitFlyCharConn then CommandToggles.SitFlyCharConn:Disconnect(); CommandToggles.SitFlyCharConn = nil end
		local root = Services.Character and Services.Character:FindFirstChild("HumanoidRootPart")
		if root then
			for _, v in ipairs(root:GetChildren()) do
				if v.Name == "SFVelocity" or v.Name == "SFAlign" or v.Name == "SFAttachment" then v:Destroy() end
		...
		local hum = Services.Character and Services.Character:FindFirstChildWhichIsA("Humanoid")
		if hum then hum.Sit = false; hum.PlatformStand = false end
		UIFuncs.SendNotification("BlueberryCmd", "Fly turned off.", 2)
	end
)

Command.Add(
	{"InfIniteJump", "InfJump"},
	{"InfIniteJump / InfJump", "Realistic infinite jump with cooldown like Roblox jump button"},
	{"None"},
	function()
		CommandToggles.InfIniteJump = true
		local JumpCooldown = false
		Services.UserInputService.JumpRequest:Connect(function()
			if not CommandToggles.InfIniteJump then return end
			if JumpCooldown then return end
			local Humanoid = Services.Character and Services.Character:FindFirstChildOfClass("Humanoid")
			if not Humanoid then return end
			JumpCooldown = true
		...
		UIFuncs.SendNotification("BlueberryCmd", "InfiniteJump on. Type ;uninfjump to stop.", 3, "success")
	end
)
...
Command.Add(
	...
	function(...)
		for i, Player in next, PlayerFuncs.GetPlayer(...) do
			if Player.Character and Player.Character.Humanoid then
				UIFuncs.SendNotification("BlueberryCmd", "Viewing @" .. Player.Name, 5)
...
Command.Add(
	{"Prefix", "Pre"},
	{"Prefix / Pre", "Changes the prefix of BlueberryCmd"},
	{"Prefix"},
	function(Prefix)
...
Command.Add(
	...
	function(Power)
		...
		UIFuncs.SendNotification("BlueberryCmd", "Touchfling turned on.", 3, "success")
...
Command.Add(
	...
	{"None"},
	function()
		UIFuncs.SendNotification("BlueberryCmd", "Touchfling turned off.", 2)
...
Command.Add(
	...
	{"None"},
	function()
		UIFuncs.SendNotification("BlueberryCmd", "Dropping all tools.", 3)
...
Command.Add(
	...
	{"None"},
	function()
		UIFuncs.SendNotification("BlueberryCmd", "LoopGrabDelete turned on.", 3, "success")
...
Command.Add(
	...
	{"None"},
	function()
		UIFuncs.SendNotification("BlueberryCmd", "LoopGrabDelete turned off.", 2)
		CommandToggles.LoopGrabDelete = false
	end
)
...
Command.Add(
	...
	{"None"},
	function()
		UIFuncs.SendNotification("BlueberryCmd", "LoopGrab turned on.", 3, "success")
...
Command.Add(
	...
	{"None"},
	function()
		UIFuncs.SendNotification("BlueberryCmd", "LoopGrab turned off.", 2)
		CommandToggles.LoopGrab = false
	end
)
...
Command.Add(
	{"LoopWalkspeed", "LWS", "LoopWalk"},
	{"LoopWalkspeed / LoopWalk / LWS", "Loops walkspeed so it re-applies if the game resets it or you die"},
	{"Number"},
	function(Number)
		local value = tonumber(Number)
		if not value then
			UIFuncs.SendNotification("BlueberryCmd", "Please enter a valid number.", 3, "error")
			return
		end
		CommandToggles.LoopWalkspeed = true
		CommandToggles.LoopWalkspeedValue = value
		UIFuncs.SendNotification("BlueberryCmd", "LoopWalkspeed set to " .. value .. ". Type ;unlws to stop.", 3, "success")
		spawn(function()
			while CommandToggles.LoopWalkspeed do
				Services.RunService.Heartbeat:Wait()
				pcall(function()
					local hum = Services.Character and Services.Character:FindFirstChildOfClass("Humanoid")
					if hum then hum.WalkSpeed = CommandToggles.LoopWalkspeedValue end
				end)
			end
		end)
	end
...
Command.Add(
	{"UnLoopWalkspeed", "UNLWS", "UnLoopWalk"},
	{"UnLoopWalkspeed / UnLoopWalk / UNLWS", "Stops the walkspeed loop"},
	{"None"},
	function()
		CommandToggles.LoopWalkspeed = false
		UIFuncs.SendNotification("BlueberryCmd", "LoopWalkspeed turned off.", 2)
	end
)
...
Command.Add(
	{"LoopJumpPower", "LJP", "LoopJump"},
	{"LoopJumpPower / LoopJump / LJP", "Loops jump power so it re-applies if the game resets it or you die"},
	{"Number"},
	function(Number)
		local value = tonumber(Number)
		if not value then
		...
		CommandToggles.LoopJumpPower = true
		CommandToggles.LoopJumpPowerValue = value
		UIFuncs.SendNotification("BlueberryCmd", "LoopJumpPower set to " .. value .. ". Type ;unljp to stop.", 3, "success")
		spawn(function()
			while CommandToggles.LoopJumpPower do
				Services.RunService.Heartbeat:Wait()
				pcall(function()
					local hum = Services.Character and Services.Character:FindFirstChildOfClass("Humanoid")
					if hum then
						if hum.UseJumpPower then
							hum.JumpPower = CommandToggles.LoopJumpPowerValue
						else
							hum.JumpHeight = (CommandToggles.LoopJumpPowerValue * CommandToggles.LoopJumpPowerValue) / (2 * workspace.Gravity)
						end
					end
				end)
			end
...
Command.Add(
	{"UnLoopJumpPower", "UNLJP", "UnLoopJump"},
	{"UnLoopJumpPower / UnLoopJump / UNLJP", "Stops the jump power loop"},
	{"None"},
	function()
		CommandToggles.LoopJumpPower = false
		UIFuncs.SendNotification("BlueberryCmd", "LoopJumpPower turned off.", 2)
	end
)
Command.Add(
	{"SitFly", "SF"},
	{"SitFly / SF [speed]", "Fly while sitting (harder to detect). ;sitfly 30 to set speed"},
	{"Speed (optional)"},
	function(Speed)
		local newSpeed = tonumber(Speed)
		if CommandToggles.SitFly then
			if newSpeed then
				CommandToggles.SitFlySpeed = newSpeed
				UIFuncs.SendNotification("BlueberryCmd", "SitFly speed set to " .. newSpeed, 3, "success")
			else
				UIFuncs.SendNotification("BlueberryCmd", "SitFly is already on. Type ;unsitfly to stop.", 3, "warning")
			end
			return
		end
		if newSpeed then CommandToggles.SitFlySpeed = newSpeed end
		CommandToggles.SitFly = true

		local function createSitFly(root, char)
			if not root then return end
			for _, v in ipairs(root:GetChildren()) do
				if v.Name == "SFVelocity" or v.Name == "SFAlign" or v.Name == "SFAttachment" then
					v:Destroy()
				end
			end
			local att = Instance.new("Attachment")
			att.Name = "SFAttachment"
			att.Parent = root
		...
		createSlFly = createSitFly

		createSlFly(
			Services.Character and Services.Character:FindFirstChild("HumanoidRootPart"),
			Services.Character
		)
		...
		CommandToggles.SitFlyCharConn = Services.LocalPlayer.CharacterAdded:Connect(function(char)
			task.wait(0.15)
			if CommandToggles.SitFly then
				createSlFly(char:FindFirstChild("HumanoidRootPart"), char)
			end
		end)
		...
		CommandToggles.SitFlyRSConn = Services.RunService.RenderStepped:Connect(function()
			...
			if lv and ao and hum then
				...
				if move.Magnitude > 0 then move = move.Unit end
				lv.VectorVelocity = move * CommandToggles.SitFlySpeed
			end
		end)

		UIFuncs.SendNotification("BlueberryCmd", "SitFly on. Speed: " .. CommandToggles.SitFlySpeed .. ". Type ;unsitfly to stop.", 4, "success")
	end
)

Command.Add(
	{"UnSitFly", "UnSF"},
	{"UnSitFly / UnSF", "Stops SitFly"},
	{"None"},
	function()
		CommandToggles.SitFly = false
		if CommandToggles.SitFlyRSConn then
			CommandToggles.SitFlyRSConn:Disconnect()
			CommandToggles.SitFlyRSConn = nil
		end
		if CommandToggles.SitFlyCharConn then
			CommandToggles.SitFlyCharConn:Disconnect()
			CommandToggles.SitFlyCharConn = nil
		end
		local root = Services.Character and Services.Character:FindFirstChild("HumanoidRootPart")
		if root then
			for _, v in ipairs(root:GetChildren()) do
		...
		UIFuncs.SendNotification("BlueberryCmd", "SitFly turned off.", 2)
	end
)
...
Command.Add(
	{"CommandCount", "CC"},
	{"CommandCount / CC", "Tells the amount of commands BlueberryCmd has"},
	{"None"},
	function()
		UIFuncs.SendNotification("BlueberryCmd", tostring(CommandAmount), 5)
	end
)
...
Command.Add(
	{"Admin", "A"},
	{"Admin / A", "Gives another user access to use BlueberryCmd commands without having it ran"},
	{"Username"},
	function(Username)
		...
		for i, Plr in next, Player do
			if not Settings.Admins[Plr.UserId] then
				...
				UIFuncs.SendNotification("BlueberryCmd", "Succcessfully gave <b>" .. Plr.Name .. "</b> admin!", 5)
				PlayerFuncs.Chat("/w " .. Plr.Name .. " [NA] You are now an admin")
			else
				UIFuncs.SendNotification("BlueberryCmd", "This user seems to already be an admin", 5)
			end
		end
	end
)
	...
	function(Username)
		...
		for i, Plr in next, Player do
			if Settings.Admins[Plr.UserId] then
				...
				UIFuncs.SendNotification("BlueberryCmd", "Succcessfully removed <b>" .. Plr.Name .. "'s</b> admin", 5)
				PlayerFuncs.Chat("/w " .. Plr.Name .. " [NA] You are no longer an admin")
			else
				UIFuncs.SendNotification("BlueberryCmd", "The user <b>" .. Plr.Name .. "</b> isn't an admin", 5)
			end
		end
	end
)
...
Command.Add(
	{"Introduction", "Int"},
	{"Introduction / Int", "An introduction to BlueberryCmd if you haven't read it yet"},
	{"None"},
	function()
		wait(0.3)
...
Command.Add(
	...
	function()
		...
		if CommandToggles.AlreadyStartedLogging == false then
			...
			UIFuncs.SendNotification("BlueberryCmd", "Logging started.", 3, "success")
			for i, Player in next, Services.Players:GetPlayers() do
				if Services.TextChatService:FindFirstChild("TextChannels") then
...
Command.Add(
	...
	{"Settings / Options / Set / Opt", "Opens the Options tab, allows you to change things about BlueberryCmd"},
	{"None"},
	function()
		local Popup = UIFuncs
...
if Methods.CheckIfVulnerable() then
	...
	UIFuncs.SendNotification(
		"BlueberryCmd",
		"Seems like this game has an remote that is vulnerable to attacks using BlueberryCmd, you have been given extra commands run <b>cmds</b> and look for commands that have '(Vuln)' next to them, to see what new commands you now have.",
		15
	)
...
-- Move autofill above the cmdbar for mobile (so it shows on top not below)
pcall(function()
	CommandAutoFill.Position = UDim2.new(0, 0, 0, -145)
	CommandAutoFill.AnchorPoint = Vector2.new(0, 0)
end)
...
GUI.Tween = function(Object, Style, Direction, Duration, Goal)
	local Info = TweenInfo.new(Duration, Enum.EasingStyle[Style], Enum.EasingDirection[Direction])
	local Tween = Services.TweenService:Create(Object, Info, Goal)
	Tween:Play()
	return Tween
end
...
-- ============================================================
-- BB BUTTON - gemoy bounce animation on press
-- ============================================================
local _bbAnimLock = false

local function bbBounce(onDone)
	-- squish down then pop back with overshoot
	if _bbAnimLock then return end
	_bbAnimLock = true
	local origSize = UDim2.new(0, 38, 0, 38)
	local t1 = Services.TweenService:Create(Button,
		TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
		{Size = UDim2.new(0, 30, 0, 30)}
	)
	t1:Play()
	t1.Completed:Connect(function()
		local t2 = Services.TweenService:Create(Button,
...
Button.MouseButton1Click:Connect(function()
	bbBounce(function()
		...
		CommandInput.Text = ""
		CommandInput:CaptureFocus()
		task.spawn(function() task.wait(); CommandInput.Text = "" end)
	end)
end)
...
UIFuncs.SendNotification("BlueberryCmd", "Loaded! Type ;cmds for commands.", 5, "success")
...
if getgenv then
	getgenv().BlueberryCmdLoaded = true
	getgenv().BlueberryCmdInterfacePath = Interface
...
-- BlueberryCmd: auto re-apply LoopWalkspeed & LoopJumpPower on respawn/death
Services.LocalPlayer.CharacterAdded:Connect(function(NewCharacter)
	local Humanoid = NewCharacter:WaitForChild("Humanoid", 10)
	if not Humanoid then return end
	task.wait(0.15)

	if CommandToggles.LoopWalkspeed then
		Humanoid.WalkSpeed = CommandToggles.LoopWalkspeedValue
		UIFuncs.SendNotification("BlueberryCmd", "LoopWalkspeed re-applied.", 2, "success")
		spawn(function()
			while CommandToggles.LoopWalkspeed do
				Services.RunService.Heartbeat:Wait()
				pcall(function()
	...
	if CommandToggles.LoopJumpPower then
		if Humanoid.UseJumpPower then
			Humanoid.JumpPower = CommandToggles.LoopJumpPowerValue
		else
			Humanoid.JumpHeight = (CommandToggles.LoopJumpPowerValue * CommandToggles.LoopJumpPowerValue) / (2 * workspace.Gravity)
		end
		UIFuncs.SendNotification("BlueberryCmd", "LoopJumpPower re-applied.", 2, "success")
		spawn(function()
			while CommandToggles.LoopJumpPower do
				Services.RunService.Heartbeat:Wait()
				pcall(function()
