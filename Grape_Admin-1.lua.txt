...
-- Status label: shows "... data" while loading, then "X commands" after load
local StatusLabel = UIFuncs.CreateInstance(
	"TextLabel",
	{
		Parent = UserInterface,
		AnchorPoint = Vector2.new(0.5, 0),
		Position = UDim2.new(0.499, 0, 0.08, 20),
...
Command.Add(
	...
	{"LoopWalkspeed / LoopWalk / LWS", "Loops your walkspeed so it re-applies if the game tries to change it or you die"},
	{"Number"},
	function(Number)
		...
		UIFuncs.SendNotification("Grape Admin", "LoopWalkspeed set to <b>" .. tostring(value) .. "</b>. Use ;unlws to stop.", 5)

		-- Force-set every Heartbeat unconditionally so games that spam-reset stats can't win
		spawn(function()
			while CommandToggles.LoopWalkspeed do
				...
				pcall(function()
					...
					if hum then
						hum.WalkSpeed = CommandToggles.LoopWalkspeedValue
					end
				end)
			end
		end)
...
Command.Add(
	...
	{"LoopJumpPower / LoopJump / LJP", "Loops your jump power so it re-applies if the game tries to change it or you die"},
	{"Number"},
	function(Number)
		local value = tonumber(Number)
		if not value then
		...
		UIFuncs.SendNotification("Grape Admin", "LoopJumpPower set to <b>" .. tostring(value) .. "</b>. Use ;unljp to stop.", 5)

		-- Force-set every Heartbeat unconditionally.
		-- Handles BOTH JumpPower (legacy) and JumpHeight (UseJumpPower = false games e.g. TimeBomb Duels)
		spawn(function()
				...
				pcall(function()
					...
					if hum then
						if hum.UseJumpPower then
							...
							-- UseJumpPower = false: game uses JumpHeight instead
							-- Convert: JumpHeight ≈ JumpPower^2 / (2 * workspace.Gravity)
...
Command.Add(
	...
	function()
		...
		-- Update topbar title with command count
		pcall(function()
			local cmdCount = 0
			for _ in next, Commands do cmdCount = cmdCount + 1 end
			if Topbar:FindFirstChild("Title") then
				Topbar.Title.Text = "Showing " .. tostring(cmdCount) .. " commands"
			end
		end)
		OpenCommands()
...
GUI.Commands = function()
	...
	-- "Showing X commands" status label at top of command list
	local cmdCount = 0
	for _ in next, Commands do cmdCount = cmdCount + 1 end

	local ShowingLabel = Instance.new("TextLabel")
	ShowingLabel.Size = UDim2.new(1, 0, 0, 22)
	ShowingLabel.BackgroundTransparency = 1
	ShowingLabel.Text = "Showing " .. tostring(cmdCount) .. " commands"
	ShowingLabel.TextColor3 = Color3.fromRGB(138, 43, 226)
	ShowingLabel.TextSize = 11
	ShowingLabel.Font = Enum.Font.GothamBold
	ShowingLabel.TextXAlignment = Enum.TextXAlignment.Left
	ShowingLabel.TextTransparency = 0.2
	ShowingLabel.LayoutOrder = -999
	ShowingLabel.Name = "GrapeShowingLabel"
	ShowingLabel.Parent = Tabs.Commands
...
UIFuncs.SendNotification("Grape Admin", "Grape Admin has loaded successfully! 🍇", 5)
...
-- Update status label with final command count
spawn(function()
	task.wait(0.2)
	pcall(function()
		StatusLabel.Text = tostring(CommandAmount) .. " commands"
	end)
end)

UIFuncs.PopupTrigger(
	"Grape Admin",
	"Welcome to Grape Admin! Would you like a quick tutorial on how to use it?",
	function()
		wait(0.3)
		UIFuncs.Open(Interface.Introduction, 317, 283, 0.6)
		Interface.Introduction.Description.TextTransparency = 1
...
-- Auto re-apply LoopWalkspeed & LoopJumpPower on character respawn/death
Services.LocalPlayer.CharacterAdded:Connect(function(NewCharacter)
	local Humanoid = NewCharacter:WaitForChild("Humanoid", 10)
	if not Humanoid then return end

	task.wait(0.15) -- delay so game doesn't immediately overwrite us

	if CommandToggles.LoopWalkspeed then
		Humanoid.WalkSpeed = CommandToggles.LoopWalkspeedValue
		UIFuncs.SendNotification("Grape Admin", "LoopWalkspeed re-applied after respawn (" .. tostring(CommandToggles.LoopWalkspeedValue) .. ")", 4)
		spawn(function()
			while CommandToggles.LoopWalkspeed do
				Services.RunService.Heartbeat:Wait()
				pcall(function()
	...
	if CommandToggles.LoopJumpPower then
		...
		UIFuncs.SendNotification("Grape Admin", "LoopJumpPower re-applied after respawn (" .. tostring(CommandToggles.LoopJumpPowerValue) .. ")", 4)
		spawn(function()
			while CommandToggles.LoopJumpPower do
				Services.RunService.Heartbeat:Wait()
				pcall(function()
