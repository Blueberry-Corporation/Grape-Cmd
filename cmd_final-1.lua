	...
	Classes = {
		["TextBox"] = function(Label)
			if Label.Name ~= "Recommend" then
				...
				-- hide inline grey recommend, shown above bar instead
...
Command.Add = function(Information)
	...
	local Entry = ({ Aliases, Description, Arguments, Plugin, Task });
	Commands[Lower(Aliases[1])] = Entry;

	-- Auto-register to autofill if UI is already loaded
	if Fill and Fill.Add and Autofill and Autofill.Example then
		pcall(Fill.Add, Entry);
	end
end
...
--> Mobile-Friendly Recommend Label (shown ABOVE commandbar)
local RecommendGui = Instance.new("ScreenGui")
RecommendGui.Name = "CMD_RecommendGui"
RecommendGui.ResetOnSpawn = false
RecommendGui.IgnoreGuiInset = true
RecommendGui.DisplayOrder = 999
RecommendGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
RecommendGui.Parent = PlayerGui

local RecommendFrame = Instance.new("Frame")
RecommendFrame.Name = "RecommendFrame"
RecommendFrame.AnchorPoint = Vector2.new(0.5, 0)
RecommendFrame.Position = UDim2.new(0.5, 0, 0, 60)
RecommendFrame.Size = UDim2.new(0.85, 0, 0, 40)
RecommendFrame.BackgroundColor3 = Color3.fromRGB(18, 22, 30)
RecommendFrame.BackgroundTransparency = 0.2
RecommendFrame.BorderSizePixel = 0
RecommendFrame.Visible = false
RecommendFrame.Parent = RecommendGui

local RecommendCorner = Instance.new("UICorner")
RecommendCorner.CornerRadius = UDim.new(0, 8)
RecommendCorner.Parent = RecommendFrame

local RecommendStroke = Instance.new("UIStroke")
RecommendStroke.Color = Color3.fromRGB(210, 140, 90)
RecommendStroke.Thickness = 1.5
RecommendStroke.Transparency = 0.5
RecommendStroke.Parent = RecommendFrame

local RecommendPrefix = Instance.new("TextLabel")
RecommendPrefix.Name = "Prefix"
RecommendPrefix.AnchorPoint = Vector2.new(0, 0.5)
RecommendPrefix.Position = UDim2.new(0, 12, 0.5, 0)
RecommendPrefix.Size = UDim2.new(0, 60, 1, 0)
RecommendPrefix.BackgroundTransparency = 1
RecommendPrefix.Text = "Tab →"
RecommendPrefix.TextColor3 = Color3.fromRGB(210, 140, 90)
RecommendPrefix.Font = Enum.Font.GothamBold
RecommendPrefix.TextSize = 13
RecommendPrefix.TextXAlignment = Enum.TextXAlignment.Left
RecommendPrefix.Parent = RecommendFrame

local RecommendText = Instance.new("TextLabel")
RecommendText.Name = "RecommendText"
RecommendText.AnchorPoint = Vector2.new(0, 0.5)
RecommendText.Position = UDim2.new(0, 75, 0.5, 0)
RecommendText.Size = UDim2.new(1, -85, 1, 0)
RecommendText.BackgroundTransparency = 1
RecommendText.Text = ""
RecommendText.TextColor3 = Color3.fromRGB(235, 240, 250)
RecommendText.Font = Enum.Font.Gotham
RecommendText.TextSize = 14
RecommendText.TextXAlignment = Enum.TextXAlignment.Left
RecommendText.TextTruncate = Enum.TextTruncate.AtEnd
RecommendText.Parent = RecommendFrame

-- Tap on recommend bar = autocomplete (mobile support)
local RecommendButton = Instance.new("TextButton")
RecommendButton.Name = "TapToComplete"
RecommendButton.Size = UDim2.new(1, 0, 1, 0)
RecommendButton.BackgroundTransparency = 1
RecommendButton.Text = ""
RecommendButton.Parent = RecommendFrame
...
-- Update recommend label above bar whenever Recommend.Text changes
local function UpdateRecommendLabel()
	local Text = Recommend.Text
	if Text and #Text > 0 then
		RecommendText.Text = Text
		RecommendFrame.Visible = true
	else
		RecommendText.Text = ""
		RecommendFrame.Visible = false
	end
...
-- Wrap Fill.Recommend to also update the label above
local _OriginalFill = Fill.Recommend
Fill.Recommend = function(InputText)
	_OriginalFill(InputText)
...
Connect(Input.FocusLost, function()
	...
	CommandBar.Visible = false
	-- Hide mobile recommend bar too
	if RecommendFrame then RecommendFrame.Visible = false end
end)
...
-- ============================================================
-- PORTED FROM OPADMIN (241 commands)
-- ============================================================
-- ported commands from opadmin to cmd format
-- these are commands that exist in opadmin but NOT in cmd
-- auto-generated port
...
Command.Add({
	...
	Task = function(Speed)
		...
		return "Speed", Format("Set walkspeed to %s", Speed)
	end,
})
...
Command.Add({
	...
	Task = function(Power)
		...
		return "JumpPower", Format("Set jump power to %s", Power)
	end,
})

Command.Add({
	Aliases = { "loopjumppower", "loopjp" },
	Description = "Loops your jumppower",
	Arguments = {
		{ Name = "Power", Type = "Number" },
	},
	Task = function(Power)
		Power = tonumber(Power) or 50
		Refresh("LoopJP", true)
		Cleaner:Add("LoopJP", Connect(Services.Run.Heartbeat, function()
			if not Get("LoopJP") then return end
			Humanoid.UseJumpPower = true
			Humanoid.JumpPower = Power
		end))
		return "LoopJP", Format("Looping jumppower: %s", Power)
	end,
})

Command.Add({
	Aliases = { "unloopjumppower", "unloopjp" },
	Description = "Stops loop jumppower",
	Arguments = {},
	Task = function()
		Add("LoopJP", false)
		return "LoopJP", "Disabled loop jumppower"
	end,
})

Command.Add({
	Aliases = { "loopwalkspeed", "loopws" },
	Description = "Loops your walkspeed",
	Arguments = {
		{ Name = "Speed", Type = "Number" },
	},
	Task = function(Speed)
		Speed = tonumber(Speed) or 16
		Refresh("LoopWS", true)
		Cleaner:Add("LoopWS", Connect(Services.Run.Heartbeat, function()
			if not Get("LoopWS") then return end
			Humanoid.WalkSpeed = Speed
		end))
		return "LoopWS", Format("Looping walkspeed: %s", Speed)
	end,
})

Command.Add({
	Aliases = { "unloopwalkspeed", "unloopws" },
	Description = "Stops loop walkspeed",
	Arguments = {},
	Task = function()
		Add("LoopWS", false)
		return "LoopWS", "Disabled loop walkspeed"
	end,
})

Command.Add({
	Aliases = { "infjump", "infinitejump", "ij" },
	Description = "Enables infinite jump",
	Arguments = {},
	Task = function()
		if Get("InfJump") then
			Add("InfJump", false)
			return "InfJump", "Disabled infinite jump"
		end
		Add("InfJump", true)
		Cleaner:Add("InfJump", Connect(Services.Input.JumpRequest, function()
			if not Get("InfJump") then return end
			Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
		end))
		return "InfJump", "Enabled infinite jump"
	end,
})

Command.Add({
	Aliases = { "uninfjump", "uninfinitejump" },
...
Command.Add({
	...
	Task = function()
		if Get("Noclip") then
			Add("Noclip", false)
			return "Noclip", "Disabled noclip"
		end
		Add("Noclip", true)
		Cleaner:Add("Noclip", Connect(Services.Run.Stepped, function()
			if not Get("Noclip") then return end
		...
		return "Noclip", "Enabled noclip"
	end,
})

Command.Add({
	Aliases = { "unnoclip", "unc", "clip" },
...
Command.Add({
	...
	Task = function(Players)
		if Get("Follow") then
			Add("Follow", false)
			return "Follow", "Stopped following"
		end
		local Target = Players[1]
		if not Target then return end
		Add("Follow", true)
		Cleaner:Add("Follow", Connect(Services.Run.Heartbeat, function()
			if not Get("Follow") then return end
			local TargetRoot = GetRoot(Target)
			if TargetRoot then
				Root.CFrame = TargetRoot.CFrame
			end
		end))
		return "Follow", Format("Following %s", Target.Name)
	end,
})
...
Command.Add({
	...
	Task = function(Players, Distance, Speed)
		if Get("Orbit") then
			Add("Orbit", false)
			return "Orbit", "Stopped orbiting"
		end
		local Target = Players[1]
		if not Target then return end
		...
		local Angle = 0
		Add("Orbit", true)
		Cleaner:Add("Orbit", Connect(Services.Run.Heartbeat, function(dt)
			if not Get("Orbit") then return end
			local TargetRoot = GetRoot(Target)
			if TargetRoot then
		...
		return "Orbit", Format("Orbiting %s", Target.Name)
	end,
})
...
Command.Add({
	...
	Task = function(X, Y, Z)
		...
		return "TpCoords", Format("Teleported to %s, %s, %s", X, Y, Z)
	end,
})

Command.Add({
	Aliases = { "cframespeed", "cfspeed", "cfws" },
	Description = "Speed up without changing walkspeed",
	Arguments = {
		{ Name = "Speed", Type = "Number" },
	},
	Task = function(Speed)
		if Get("CFSpeed") then
			Add("CFSpeed", false)
			return "CFSpeed", "Disabled cframe speed"
		end
		Speed = tonumber(Speed) or 2
		Add("CFSpeed", true)
		Cleaner:Add("CFSpeed", Connect(Services.Run.Heartbeat, function()
			if not Get("CFSpeed") then return end
			if Humanoid.MoveDirection.Magnitude > 0 then
				Root.CFrame = Root.CFrame + Humanoid.MoveDirection * Speed
			end
		end))
		return "CFSpeed", Format("CFrame speed: %s", Speed)
	end,
})

Command.Add({
	Aliases = { "uncframespeed", "uncfspeed", "uncfws" },
	Description = "Stop cframe speed",
	Arguments = {},
	Task = function()
		Add("CFSpeed", false)
		return "CFSpeed", "Disabled cframe speed"
...
Command.Add({
	...
	Task = function(Speed)
		if Get("Swim") then
			...
			workspace.Gravity = 196.2
			return "Swim", "Disabled swim"
		end
		Speed = tonumber(Speed) or 16
		Add("Swim", true)
		workspace.Gravity = 19.62
		Humanoid:SetStateEnabled(Enum.HumanoidStateType.Swimming, true)
		Humanoid:ChangeState(Enum.HumanoidStateType.Swimming)
		Humanoid.WalkSpeed = Speed
		return "Swim", "Enabled swim"
	end,
})

Command.Add({
	Aliases = { "unswim", "stopswim" },
	Description = "Stop swimming in the air",
	Arguments = {},
	Task = function()
		Add("Swim", false)
		Humanoid:SetStateEnabled(Enum.HumanoidStateType.Swimming, false)
...
Command.Add({
	Aliases = { "freeze" },
	Description = "Freeze your character in place",
	Arguments = {},
	Task = function()
		if Get("Freeze") then
			Add("Freeze", false)
		...
		Add("Freeze", true)
		Cleaner:Add("Freeze", Connect(Services.Run.Heartbeat, function()
			if not Get("Freeze") then return end
			Root.Anchored = true
		end))
...
Command.Add({
	Aliases = { "unfreeze", "thaw" },
	Description = "Unfreeze your character",
	Arguments = {},
	Task = function()
		Add("Freeze", false)
		Root.Anchored = false
...
Command.Add({
	Aliases = { "mirror", "mimic" },
	Description = "Copy a player's movements",
	Arguments = {
		{ Name = "Player", Type = "Player" },
		{ Name = "Distance", Type = "Number" },
	},
	Task = function(Players, Distance)
		if Get("Mirror") then
			Add("Mirror", false)
			return "Mirror", "Stopped mirroring"
		end
		local Target = Players[1]
		if not Target then return end
		Distance = tonumber(Distance) or 5
		Add("Mirror", true)
		Cleaner:Add("Mirror", Connect(Services.Run.Heartbeat, function()
			if not Get("Mirror") then return end
			local TargetRoot = GetRoot(Target)
			if TargetRoot then
				Root.CFrame = TargetRoot.CFrame * CFrame.new(0, 0, Distance)
			end
		end))
		return "Mirror", Format("Mirroring %s", Target.Name)
	end,
})

Command.Add({
	Aliases = { "unmirror", "unmimic" },
...
Command.Add({
	Aliases = { "skydive" },
	Description = "Launch yourself into the sky",
	Arguments = {
		{ Name = "Height", Type = "Number" },
	},
	Task = function(Height)
		Height = tonumber(Height) or 500
		Root.CFrame = Root.CFrame + Vector3.new(0, Height, 0)
		return "Skydive", Format("Launched %s studs up", Height)
	end,
})

Command.Add({
	Aliases = { "zerovelocity", "zerovel", "novel" },
	Description = "Stop all velocity on your character",
	Arguments = {},
	Task = function()
		for _, Part in next, Character:GetDescendants() do
			if Part:IsA("BasePart") then
		...
		return "ZeroVelocity", "Velocity stopped"
	end,
})

Command.Add({
	Aliases = { "saveposition", "savepos" },
	Description = "Save your current position",
	Arguments = {},
	Task = function()
		Add("SavedPos", Root.CFrame)
		local Pos = Root.CFrame.Position
		return "SavePos", Format("Saved: %.0f, %.0f, %.0f", Pos.X, Pos.Y, Pos.Z)
	end,
})

Command.Add({
	Aliases = { "loadposition", "loadpos" },
	Description = "Load saved position",
	Arguments = {},
	Task = function()
		local Saved = Get("SavedPos")
		if Saved then
			Root.CFrame = Saved
			return "LoadPos", "Position loaded"
		end
		return "LoadPos", "No position saved"
	end,
})

Command.Add({
	Aliases = { "tpwalk", "teleportwalk" },
	Description = "Teleport when walking",
	Arguments = {
	...
	Task = function(Mult)
		if Get("TpWalk") then
			Add("TpWalk", false)
			return "TpWalk", "TpWalk disabled"
		end
		Mult = tonumber(Mult) or 3
		Add("TpWalk", true)
		Cleaner:Add("TpWalk", Connect(Services.Run.Heartbeat, function()
			if not Get("TpWalk") then return end
			if Humanoid.MoveDirection.Magnitude > 0 then
				Character:TranslateBy(Humanoid.MoveDirection * Mult)
			end
		end))
		return "TpWalk", Format("TpWalk enabled (x%s)", Mult)
	end,
})

Command.Add({
	Aliases = { "untpwalk2", "unteleportwalk" },
	Description = "Disables tpwalk",
	Arguments = {},
	Task = function()
		Add("TpWalk", false)
		return "TpWalk", "TpWalk disabled"
...
Command.Add({
	...
	Task = function()
		local Pos = Root.CFrame.Position
		return "Coords", Format("X: %.1f, Y: %.1f, Z: %.1f", Pos.X, Pos.Y, Pos.Z)
	end,
})

Command.Add({
	Aliases = { "copyposition", "copycoords", "copypos" },
	Description = "Copy your position to clipboard",
	Arguments = {},
	Task = function()
		local Pos = Root.CFrame.Position
		if setclipboard then
			setclipboard(Format("%.2f,%.2f,%.2f", Pos.X, Pos.Y, Pos.Z))
			return "CopyPos", "Position copied to clipboard"
		end
		return "CopyPos", "setclipboard not supported"
	end,
})
...
Command.Add({
	...
	Task = function(Players)
		for _, Player in next, Players do
			return "UserID", Format("%s's ID: %s", Player.Name, Player.UserId)
		end
	end,
})

Command.Add({
	Aliases = { "copyuserid", "copyid" },
	Description = "Copy a player's user ID to clipboard",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		...
		if Player and setclipboard then
			...
			return "CopyID", Format("Copied %s's ID: %s", Player.Name, Player.UserId)
		end
	end,
})
...
Command.Add({
	...
	Arguments = {},
	Task = function()
		return "PlaceID", Format("Place ID: %s", game.PlaceId)
	end,
})
...
Command.Add({
	...
	Arguments = {},
	Task = function()
		return "JobID", Format("Job ID: %s", game.JobId)
	end,
})
...
Command.Add({
	...
	Task = function()
		if setclipboard then
			...
			return "CopyPlace", Format("Copied place ID: %s", game.PlaceId)
		end
	end,
})
...
Command.Add({
	...
	Task = function()
		if setclipboard then
			...
			return "CopyJob", "Job ID copied"
		end
	end,
})

Command.Add({
	Aliases = { "age", "accountage" },
	Description = "Show account age",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		for _, Player in next, (Players and #Players > 0 and Players or { LocalPlayer }) do
			local Days = Player.AccountAge
			return "Age", Format("%s: %d years, %d months, %d days",
				Player.Name,
				math.floor(Days / 365),
...
Command.Add({
	...
	Task = function()
		...
		return "Ping", Format("Ping: %sms", Ping)
	end,
})
...
Command.Add({
	...
	Task = function(Gravity)
		Gravity = tonumber(Gravity) or 196.2
		workspace.Gravity = Gravity
		return "Gravity", Format("Gravity set to %s", Gravity)
	end,
})

Command.Add({
	Aliases = { "freemouse" },
	Description = "Unlock your mouse cursor",
	Arguments = {},
	Task = function()
		if Get("FreeMouse") then
			Add("FreeMouse", false)
			return "FreeMouse", "Free mouse disabled"
		end
		Add("FreeMouse", true)
		Cleaner:Add("FreeMouse", Connect(Services.Run.RenderStepped, function()
			if not Get("FreeMouse") then return end
			Services.Input.MouseBehavior = Enum.MouseBehavior.Default
		end))
		return "FreeMouse", "Free mouse enabled"
	end,
})

Command.Add({
	Aliases = { "fpscap", "maxfps", "unlockfps" },
	Description = "Set FPS cap",
	Arguments = {
		{ Name = "FPS", Type = "Number" },
	},
	Task = function(FPS)
		FPS = tonumber(FPS) or 999
		if setfpscap then
			setfpscap(FPS)
			return "FPSCap", Format("FPS cap set to %s", FPS)
		end
		return "FPSCap", "setfpscap not supported by your executor"
	end,
})

Command.Add({
	Aliases = { "showbounds", "hitboxes" },
	Description = "Show bounding boxes",
	Arguments = {},
	Task = function()
		settings():GetService("RenderSettings").ShowBoundingBoxes = true
		return "ShowBounds", "Bounding boxes enabled"
	end,
})

Command.Add({
	Aliases = { "unshowbounds", "unhitboxes" },
	Description = "Hide bounding boxes",
	Arguments = {},
	Task = function()
		settings():GetService("RenderSettings").ShowBoundingBoxes = false
		return "ShowBounds", "Bounding boxes disabled"
	end,
})

Command.Add({
	Aliases = { "droptools", "dropall" },
	Description = "Drop all droppable tools",
	Arguments = {},
	Task = function()
		local Count = 0
		...
		return "DropTools", Format("Dropped %d tool(s)", Count)
	end,
})

Command.Add({
	Aliases = { "notools", "removetools", "deletetools2" },
	Description = "Remove all your tools",
	Arguments = {},
	Task = function()
		local Count = 0
		for _, Tool in next, Character:GetDescendants() do
			if Tool:IsA("Tool") then Tool:Destroy(); Count += 1 end
		end
		for _, Tool in next, Backpack:GetDescendants() do
			if Tool:IsA("Tool") then Tool:Destroy(); Count += 1 end
		end
		return "NoTools", Format("Removed %d tool(s)", Count)
	end,
})

Command.Add({
	Aliases = { "gettools", "tools" },
	Description = "Steal tools from workspace",
	Arguments = {},
	Task = function()
		local Count = 0
		...
		return "GetTools", Format("Stole %d tool(s)", Count)
	end,
})

Command.Add({
	Aliases = { "nofog" },
	Description = "Remove fog from lighting",
	Arguments = {},
	Task = function()
		if Get("NoFog") then
			Add("NoFog", false)
			return "NoFog", "Fog restored"
		end
		Add("NoFog", true)
		Cleaner:Add("NoFog", Connect(Services.Run.Heartbeat, function()
			if not Get("NoFog") then return end
			Services.Lighting.FogEnd = 100000
			Services.Lighting.FogStart = 100000
		end))
		return "NoFog", "Fog removed"
	end,
})

Command.Add({
	Aliases = { "unnofog" },
	Description = "Restore fog",
	Arguments = {},
	Task = function()
		Add("NoFog", false)
		Services.Lighting.FogEnd = 100000
...
Command.Add({
	Aliases = { "fixlighting" },
	Description = "Reset lighting to defaults",
	Arguments = {},
	Task = function()
		...
		return "FixLighting", "Lighting reset"
	end,
})

Command.Add({
	Aliases = { "removesky" },
	Description = "Remove the sky",
	Arguments = {},
	Task = function()
		for _, V in next, Services.Lighting:GetChildren() do
			if V:IsA("Sky") then V:Destroy() end
		end
		return "RemoveSky", "Sky removed"
	end,
})
...
Command.Add({
	...
	Task = function(Amount)
		...
		Services.Lighting.Brightness = Amount
		return "Brightness", Format("Brightness set to %s", Amount)
	end,
})

Command.Add({
	Aliases = { "ambient" },
	Description = "Set ambient color (R,G,B 0-255)",
	Arguments = {
		{ Name = "R", Type = "Number" },
		{ Name = "G", Type = "Number" },
		{ Name = "B", Type = "Number" },
	...
	Task = function(R, G, B)
		...
		G = tonumber(G) or 0
		B = tonumber(B) or 0
		Services.Lighting.Ambient = Color3.fromRGB(R, G, B)
		return "Ambient", Format("Ambient set to %d,%d,%d", R, G, B)
	end,
})

Command.Add({
	Aliases = { "outdoorambient" },
	Description = "Set outdoor ambient color (R,G,B 0-255)",
	Arguments = {
		{ Name = "R", Type = "Number" },
		{ Name = "G", Type = "Number" },
		{ Name = "B", Type = "Number" },
	...
	Task = function(R, G, B)
		...
		G = tonumber(G) or 127
		B = tonumber(B) or 127
		Services.Lighting.OutdoorAmbient = Color3.fromRGB(R, G, B)
		return "OutdoorAmbient", Format("Outdoor ambient set to %d,%d,%d", R, G, B)
	end,
})
...
Command.Add({
	...
	Task = function()
		...
		return "Night", "Set time to night"
	end,
})
...
Command.Add({
	...
	Task = function(Message)
		...
		return "Chat", Format("Said: %s", Message)
	end,
})

Command.Add({
	Aliases = { "spamchat" },
	Description = "Spam chat with a message",
	Arguments = {
		{ Name = "Interval", Type = "Number" },
		{ Name = "Message", Type = "String" },
	},
	Task = function(Interval, Message)
		if Get("SpamChat") then
			Add("SpamChat", false)
			return "SpamChat", "Spam chat stopped"
		end
		if not Message then return "SpamChat", "Provide a message" end
		Interval = tonumber(Interval) or 0.5
		Add("SpamChat", true)
		Spawn(function()
			while Get("SpamChat") do
				pcall(function()
					Services.Chat.TextChannels.RBXGeneral:SendAsync(Message)
				end)
				Wait(Interval)
		...
		return "SpamChat", Format("Spamming: %s", Message)
	end,
})

Command.Add({
	Aliases = { "unspamchat" },
	Description = "Stop spam chat",
	Arguments = {},
	Task = function()
		Add("SpamChat", false)
		return "SpamChat", "Spam chat stopped"
...
Command.Add({
	...
	Task = function()
		...
		local Cursor = ""
		repeat
			local URL = Format("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=Asc&limit=100&cursor=%s", game.PlaceId, Cursor)
		...
		if #Servers > 0 then
			Services.Teleport:TeleportToPlaceInstance(game.PlaceId, Servers[math.random(#Servers)], LocalPlayer)
			return "ServerHop", "Hopping to new server..."
		end
		Services.Teleport:Teleport(game.PlaceId, LocalPlayer)
		return "ServerHop", "No servers found, rejoining..."
	end,
})
...
Command.Add({
	Aliases = { "fpsboost", "performancemode" },
	...
	Task = function()
		for _, V in next, workspace:GetDescendants() do
			pcall(function()
				...
				elseif V:IsA("Decal") or V:IsA("Texture") or V:IsA("ParticleEmitter")
					or V:IsA("Trail") or V:IsA("Smoke") or V:IsA("Fire") or V:IsA("Sparkles") then
					V:Destroy()
				end
			end)
		end
		...
		Services.Lighting.FogEnd = 100000
		return "FPSBoost", "FPS boost applied"
	end,
})

Command.Add({
	Aliases = { "stopreplag", "srl" },
	Description = "Set IncomingReplicationLag to -1",
	Arguments = {},
	Task = function()
		settings():GetService("NetworkSettings").IncomingReplicationLag = -1
		return "StopReplag", "Replication lag disabled"
	end,
})

Command.Add({
	Aliases = { "countcommands", "countcmds" },
	Description = "Count total commands",
	Arguments = {},
	Task = function()
		local Count = 0
		for _ in next, Commands do Count += 1 end
		return "CountCmds", Format("%d commands loaded", Count)
	end,
})

Command.Add({
	Aliases = { "datalimit" },
	Description = "Set outgoing KB/s limit",
	Arguments = {
	...
	Task = function(Limit)
		...
		return "DataLimit", Format("Data limit set to %s KB/s", Limit)
	end,
})

Command.Add({
	Aliases = { "saveinstance", "savegame" },
	Description = "Save the game instance",
	Arguments = {},
	Task = function()
		if saveinstance then
			saveinstance(game)
			return "SaveInstance", "Saving instance..."
		end
		return "SaveInstance", "saveinstance not supported"
	end,
})

Command.Add({
	Aliases = { "clickmouse", "click" },
	Description = "Click your mouse",
	Arguments = {},
	Task = function()
		if mouse1click then mouse1click() end
		return "Click", "Clicked mouse"
	end,
})

Command.Add({
	Aliases = { "deleteplayer", "delplayer" },
	Description = "Delete a player's character client-side",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		...
		return "DeletePlayer", "Character deleted"
	end,
})

Command.Add({
	Aliases = { "kick" },
	Description = "Kick a player client-side",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		for _, Player in next, Players do
			pcall(function() Player.Parent = nil end)
		end
		return "Kick", "Player kicked (client-side)"
	end,
})

Command.Add({
	Aliases = { "maxzoom", "maxz2" },
	Description = "Set max camera zoom",
	Arguments = {
		{ Name = "Distance", Type = "Number" },
	},
	Task = function(Distance)
		...
		return "MaxZoom", Format("Max zoom set to %s", Distance)
	end,
})

Command.Add({
	Aliases = { "minzoom", "minz2" },
	Description = "Set min camera zoom",
	Arguments = {
		{ Name = "Distance", Type = "Number" },
	},
	Task = function(Distance)
		...
		return "MinZoom", Format("Min zoom set to %s", Distance)
	end,
})

Command.Add({
	Aliases = { "fixcam", "resetcam" },
	Description = "Reset camera properties",
	Arguments = {},
	Task = function()
		Camera.CameraType = Enum.CameraType.Custom
		Camera.CameraSubject = Humanoid
...
Command.Add({
	Aliases = { "size" },
	Description = "Scale your character size",
	Arguments = {
		{ Name = "Size", Type = "Number" },
	},
	Task = function(Size)
		...
		return "Size", Format("Size scaled by %s", Size)
	end,
})

Command.Add({
	Aliases = { "nosit", "disablesit" },
	...
	Task = function()
		if Get("NoSit") then
			...
			return "NoSit", "Sitting re-enabled"
		end
		Add("NoSit", true)
		Humanoid:SetStateEnabled(Enum.HumanoidStateType.Seated, false)
		Humanoid.Sit = false
		return "NoSit", "Sitting disabled"
	end,
})

Command.Add({
	Aliases = { "esit", "enablesit" },
	Description = "Allow your character to sit again",
	Arguments = {},
	Task = function()
		Add("NoSit", false)
		Humanoid:SetStateEnabled(Enum.HumanoidStateType.Seated, true)
...
Command.Add({
	Aliases = { "platformstand", "pstand" },
	Description = "Enable platform stand",
	Arguments = {},
	Task = function()
		Humanoid.PlatformStand = not Humanoid.PlatformStand
		return "PlatformStand", Format("Platform stand: %s", Humanoid.PlatformStand)
	end,
})

Command.Add({
	Aliases = { "unplatformstand", "unpstand" },
	Description = "Disable platform stand",
	Arguments = {},
	Task = function()
		Humanoid.PlatformStand = false
		return "PlatformStand", "Platform stand disabled"
	end,
})

Command.Add({
	Aliases = { "animation2", "anim2" },
	Description = "Play animation by ID",
	Arguments = {
		{ Name = "ID", Type = "String" },
	},
	Task = function(ID)
		if not ID then return "Anim", "Provide an animation ID" end
		...
		Track:Play()
		return "Anim", Format("Playing animation %s", ID)
	end,
})

Command.Add({
	Aliases = { "stopanimations2", "stopanim2" },
	...
	Task = function()
		...
		return "StopAnim", "Animations stopped"
	end,
})

Command.Add({
	Aliases = { "removehats" },
	Description = "Remove all hats/accessories",
	Arguments = {},
	Task = function()
		local Count = 0
		...
		return "RemoveHats", Format("Removed %d accessory(ies)", Count)
	end,
})
...
Command.Add({
	...
	Task = function()
		...
		local Conn
		Humanoid.Health = 0
		Conn = Connect(LocalPlayer.CharacterAdded, function(Char)
			Char:WaitForChild("HumanoidRootPart").CFrame = Pos
			Conn:Disconnect()
		end)
		return "Refresh", "Refreshing character"
	end,
})

Command.Add({
	Aliases = { "strengthen", "dense" },
	Description = "Make character more dense",
	Arguments = {
		{ Name = "Density", Type = "Number" },
	},
	Task = function(Density)
		Density = tonumber(Density) or 100
		for _, Part in next, Character:GetDescendants() do
			if Part:IsA("BasePart") then
				Part.CustomPhysicalProperties = PhysicalProperties.new(Density, 0.3, 0.5)
			end
		end
		return "Strengthen", Format("Density set to %s", Density)
	end,
})

Command.Add({
	Aliases = { "weaken", "light" },
	Description = "Make character less dense",
	Arguments = {
		{ Name = "Density", Type = "Number" },
	},
	Task = function(Density)
		Density = tonumber(Density) or 0
		for _, Part in next, Character:GetDescendants() do
			if Part:IsA("BasePart") then
				Part.CustomPhysicalProperties = PhysicalProperties.new(Density, 0.3, 0.5)
			end
		end
		return "Weaken", Format("Density set to %s", Density)
	end,
})

Command.Add({
	Aliases = { "unstrengthen", "unweaken" },
	Description = "Reset character physical properties",
	Arguments = {},
	Task = function()
		for _, Part in next, Character:GetDescendants() do
			if Part:IsA("BasePart") then
		...
		return "Unstrengthen", "Physical properties reset"
	end,
})

Command.Add({
	Aliases = { "camerainvert", "invert" },
	Description = "Make character face opposite to camera",
	Arguments = {},
	Task = function()
		if Get("CamInvert") then
			Add("CamInvert", false)
			return "CamInvert", "Camera invert disabled"
		end
		Add("CamInvert", true)
		Cleaner:Add("CamInvert", Connect(Services.Run.Heartbeat, function()
			if not Get("CamInvert") then return end
			local Look = -Camera.CFrame.LookVector
			Root.CFrame = CFrame.new(Root.Position, Root.Position + Vector3.new(Look.X, 0, Look.Z))
		end))
		return "CamInvert", "Camera invert enabled"
	end,
})

Command.Add({
	Aliases = { "uncamerainvert", "uninvert" },
	Description = "Disable camera invert",
	Arguments = {},
	Task = function()
		Add("CamInvert", false)
		return "CamInvert", "Camera invert disabled"
...
Command.Add({
	...
	Task = function(Speed)
		if Get("Spin") then
			Add("Spin", false)
			if Get("SpinPart") then Get("SpinPart"):Destroy() end
			return "Spin", "Stopped spinning"
		end
		...
		Add("Spin", true)
		Add("SpinPart", BAV)
		return "Spin", Format("Spinning at %s", Speed)
	end,
})
...
Command.Add({
	...
	Task = function()
		Add("Spin", false)
		local Part = Get("SpinPart")
		if Part then Part:Destroy(); Add("SpinPart", nil) end
		return "Spin", "Stopped spinning"
	end,
})

Command.Add({
	Aliases = { "tweento", "tgoto" },
	Description = "Tween teleport to a player",
	Arguments = {
		{ Name = "Player", Type = "Player" },
		{ Name = "Duration", Type = "Number" },
	},
	Task = function(Players, Duration)
		local Target = Players and Players[1]
		if not Target then return end
		Duration = tonumber(Duration) or 1
		local CF = Instance.new("CFrameValue")
		CF.Value = Root.CFrame
		CF.Changed:Connect(function(V) Root.CFrame = V end)
		Services.Tween:Create(CF, TweenInfo.new(Duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Value = Target.Character and Target.Character:GetPivot() or Root.CFrame
		}):Play()
		return "TweenTo", Format("Tweening to %s", Target.Name)
	end,
})

Command.Add({
	Aliases = { "fakelag", "desync" },
	Description = "Create fake lag on your character",
	Arguments = {},
	Task = function()
		if Get("FakeLag") then
			Add("FakeLag", false)
			return "FakeLag", "Fake lag disabled"
		end
		Add("FakeLag", true)
		Spawn(function()
			while Get("FakeLag") do
				pcall(function()
					Root.Anchored = true
				end)
				Wait(0.1)
		...
		return "FakeLag", "Fake lag enabled"
	end,
})

Command.Add({
	Aliases = { "unfakelag", "undesync" },
	Description = "Disable fake lag",
	Arguments = {},
	Task = function()
		Add("FakeLag", false)
		Root.Anchored = false
...
Command.Add({
	...
	Description = "Launch yourself like a rocket",
	Arguments = {
		{ Name = "Power", Type = "Number" },
	},
	Task = function(Power)
		...
		return "Rocket", Format("Launched with power %s", Power)
	end,
})
...
Command.Add({
	...
	Task = function(Power, Duration)
		Power = tonumber(Power) or 100
		Duration = tonumber(Duration) or 0.5
		local BV = Instance.new("BodyVelocity")
		BV.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
		BV.Velocity = Root.CFrame.LookVector * Power
		BV.Parent = Root
		game:GetService("Debris"):AddItem(BV, Duration)
		return "Push", Format("Pushed with power %s", Power)
	end,
})

Command.Add({
	Aliases = { "explode", "explosion" },
	...
	Task = function(Size)
		...
		Exp.Position = Root.Position - Vector3.new(0, 1, 0)
		Exp.BlastRadius = Size
		Exp.BlastPressure = 500000
		Exp.DestroyJointRadiusPercent = 0
		Exp.Parent = workspace
		return "Explode", Format("Exploded with size %s", Size)
	end,
})

Command.Add({
	Aliases = { "clone2" },
	Description = "Clone your character",
	Arguments = {},
	Task = function()
		Character.Archivable = true
		...
		return "Clone", "Character cloned"
	end,
})

Command.Add({
	Aliases = { "removeclones", "clearclones" },
	Description = "Remove all clones of your character",
	Arguments = {},
	Task = function()
		local Count = 0
		...
		return "RemoveClones", Format("Removed %d clone(s)", Count)
	end,
})
...
Command.Add({
	Aliases = { "spaz", "seizure" },
	Description = "Make your character spaz out",
	Arguments = {},
	Task = function()
		if Get("Spaz") then
			Add("Spaz", false)
			return "Spaz", "Spaz disabled"
		end
		Add("Spaz", true)
		Cleaner:Add("Spaz", Connect(Services.Run.Heartbeat, function()
			if not Get("Spaz") then return end
			pcall(function()
		...
		return "Spaz", "Spaz enabled"
	end,
})

Command.Add({
	Aliases = { "unspaz", "unseizure" },
	Description = "Stop spaz",
	Arguments = {},
	Task = function()
		Add("Spaz", false)
		return "Spaz", "Spaz disabled"
...
Command.Add({
	Aliases = { "annoy" },
	Description = "Teleport spam around a player",
	Arguments = {
		{ Name = "Player", Type = "Player" },
		{ Name = "Speed", Type = "Number" },
	},
	Task = function(Players, Speed)
		if Get("Annoy") then
			Add("Annoy", false)
			return "Annoy", "Stopped annoying"
		end
		local Target = Players and Players[1]
		if not Target then return end
		Speed = tonumber(Speed) or 0.1
		Add("Annoy", true)
		local Positions = {
			CFrame.new(5,0,0), CFrame.new(-5,0,0),
			CFrame.new(0,5,0), CFrame.new(0,0,5),
			CFrame.new(0,0,-5), CFrame.new(3,3,3),
		}
		local Idx = 1
		Cleaner:Add("Annoy", Connect(Services.Run.Heartbeat, function()
			if not Get("Annoy") then return end
			local TR = GetRoot(Target)
			if TR then
				pcall(function() Root.CFrame = TR.CFrame * Positions[Idx] end)
				Idx = Idx % #Positions + 1
			end
		end))
		return "Annoy", Format("Annoying %s", Target.Name)
	end,
})

Command.Add({
	Aliases = { "unannoy" },
	Description = "Stop annoying",
	Arguments = {},
	Task = function()
		Add("Annoy", false)
		return "Annoy", "Stopped annoying"
...
Command.Add({
	Aliases = { "spook", "jumpscare" },
	Description = "Teleport briefly in front of someone",
	Arguments = {
		{ Name = "Player", Type = "Player" },
		{ Name = "Duration", Type = "Number" },
	},
	Task = function(Players, Duration)
		...
		for _, Player in next, Players do
			local TR = GetRoot(Player)
			if TR then
				Root.CFrame = TR.CFrame * CFrame.new(0, 0, -3) * CFrame.Angles(0, math.rad(180), 0)
				Wait(Duration)
				Root.CFrame = OldCF
			end
		end
		return "Spook", "Spooked!"
	end,
})

Command.Add({
	Aliases = { "robang", "bang" },
	Description = "Robang a player",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		if Get("Robang") then
			Add("Robang", false)
			return "Robang", "Stopped robang"
		end
		local Target = Players and Players[1]
		if not Target then return end
		Add("Robang", true)
		Cleaner:Add("Robang", Connect(Services.Run.Stepped, function()
			if not Get("Robang") then return end
			pcall(function()
				local Head = Target.Character and Target.Character:FindFirstChild("Head")
				if Head then
					local CF = CFrame.new(0, 1.5, -1.5 - math.sin(os.clock() * 15)) * CFrame.Angles(0, math.rad(180), 0)
					Root.CFrame = Head.CFrame * CF
				end
			end)
		end))
		return "Robang", Format("Banging %s", Target.Name)
	end,
})

Command.Add({
	Aliases = { "stoprobang", "unrobang" },
	Description = "Stop robang",
	Arguments = {},
	Task = function()
		Add("Robang", false)
		return "Robang", "Stopped robang"
...
Command.Add({
	Aliases = { "netlag" },
	Description = "Netlag a player",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		for _, Target in next, Players do
			local ID = "NetLag_" .. Target.Name
			Cleaner:Add(ID, Connect(Services.Run.Heartbeat, function()
		...
		return "NetLag", "Netlagging targets"
	end,
})

Command.Add({
	Aliases = { "unnetlag" },
	Description = "Stop netlagging",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		if Players and #Players > 0 then
			for _, P in next, Players do
				Cleaner:Clean("NetLag_" .. P.Name)
			end
		end
		return "NetLag", "Stopped netlagging"
	end,
})

Command.Add({
	Aliases = { "loopoof", "oofloop" },
	Description = "Loop character sounds for all players",
	Arguments = {},
	Task = function()
		if Get("LoopOof") then
			Add("LoopOof", false)
			return "LoopOof", "Loop oof disabled"
		end
		Add("LoopOof", true)
		Cleaner:Add("LoopOof", Connect(Services.Run.Heartbeat, function()
			if not Get("LoopOof") then return end
			for _, Plr in next, Services.Players:GetPlayers() do
				local Head = Plr.Character and Plr.Character:FindFirstChild("Head")
				if Head then
					for _, Sound in next, Head:GetChildren() do
		...
		return "LoopOof", "Loop oof enabled"
	end,
})
...
Command.Add({
	Aliases = { "copychat" },
	Description = "Spy on player chat",
	Arguments = {},
	Task = function()
		if Get("CopyChat") then
			Add("CopyChat", false)
			Cleaner:Clean("CopyChat")
			return "CopyChat", "Chat spy disabled"
		end
		Add("CopyChat", true)
		for _, Plr in next, Services.Players:GetPlayers() do
			if Plr ~= LocalPlayer then
				Cleaner:Add("CopyChat", Connect(Plr.Chatted, function(Msg)
					print(Format("[ChatSpy] %s: %s", Plr.Name, Msg))
				end))
			end
		end
		return "CopyChat", "Chat spy enabled (check console)"
	end,
})

Command.Add({
	Aliases = { "uncopychat" },
	Description = "Disable chat spy",
	Arguments = {},
	Task = function()
		Add("CopyChat", false)
		Cleaner:Clean("CopyChat")
...
Command.Add({
	Aliases = { "antivoid", "novoid" },
	Description = "Stop the void from killing you",
	Arguments = {},
	Task = function()
		if Get("AntiVoid") then
			Add("AntiVoid", false)
			return "AntiVoid", "Anti-void disabled"
		end
		Add("AntiVoid", true)
		Cleaner:Add("AntiVoid", Connect(Services.Run.Heartbeat, function()
			if not Get("AntiVoid") then return end
			pcall(function()
				local DestroyHeight = workspace.FallenPartsDestroyHeight
				if Root.Position.Y <= DestroyHeight + 30 then
					Root.CFrame = Root.CFrame + Vector3.new(0, math.abs(DestroyHeight) + 50, 0)
				end
			end)
		end))
		return "AntiVoid", "Anti-void enabled"
	end,
})

Command.Add({
	Aliases = { "unantivoid" },
	Description = "Disable anti-void",
	Arguments = {},
	Task = function()
		Add("AntiVoid", false)
		return "AntiVoid", "Anti-void disabled"
...
Command.Add({
	Aliases = { "autopickup", "apickup" },
	Description = "Auto pick up tools",
	Arguments = {
		{ Name = "Range", Type = "Number" },
	},
	Task = function(Range)
		if Get("AutoPickup") then
			Add("AutoPickup", false)
			return "AutoPickup", "Auto pickup disabled"
		end
		if not firetouchinterest then
			return "AutoPickup", "firetouchinterest not available"
		end
		Range = tonumber(Range) or 10
		Add("AutoPickup", true)
		Cleaner:Add("AutoPickup", Connect(Services.Run.Heartbeat, function()
			if not Get("AutoPickup") then return end
			for _, V in next, workspace:GetDescendants() do
				if V:IsA("Tool") and V:FindFirstChild("Handle") then
					if (Root.Position - V.Handle.Position).Magnitude <= Range then
		...
		return "AutoPickup", Format("Auto pickup enabled (range: %s)", Range)
	end,
})

Command.Add({
	Aliases = { "unautopickup" },
	Description = "Disable auto pickup",
	Arguments = {},
	Task = function()
		Add("AutoPickup", false)
		return "AutoPickup", "Auto pickup disabled"
...
Command.Add({
	Aliases = { "hitbox", "expandhitbox" },
	Description = "Make player hitboxes bigger",
	Arguments = {
		{ Name = "Size", Type = "Number" },
	},
	Task = function(Size)
		if Get("Hitbox") then
			for Head, Original in next, Get("HitboxOriginals") or {} do
			...
			Add("Hitbox", false)
			Add("HitboxOriginals", nil)
			return "Hitbox", "Hitbox disabled"
		end
		Size = math.clamp(tonumber(Size) or 10, 1, 50)
		Add("Hitbox", true)
		Add("HitboxOriginals", {})
		Cleaner:Add("Hitbox", Connect(Services.Run.Heartbeat, function()
			if not Get("Hitbox") then return end
			for _, Plr in next, Services.Players:GetPlayers() do
				if Plr == LocalPlayer then continue end
				local Head = Plr.Character and Plr.Character:FindFirstChild("Head")
				if Head then
					local Originals = Get("HitboxOriginals")
					if not Originals[Head] then
						Originals[Head] = { Size = Head.Size, Transparency = Head.Transparency }
					end
		...
		return "Hitbox", Format("Hitbox enabled (size: %s)", Size)
	end,
})

Command.Add({
	Aliases = { "unhitbox" },
	Description = "Disable hitbox expansion",
	Arguments = {},
	Task = function()
		for Head, Original in next, Get("HitboxOriginals") or {} do
			if Head and Head.Parent then
...
Command.Add({
	Aliases = { "fling2" },
	Description = "Fling a player using velocity",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		for _, Target in next, Players do
			...
			local OldCF = Root.CFrame
			local ID = "Fling_" .. Target.Name
			Cleaner:Add(ID, Connect(Services.Run.Heartbeat, function()
				local Vel = TR.AssemblyLinearVelocity
				if Vel.Magnitude < 100 then
					Root.CFrame = TR.CFrame
					Root.AssemblyLinearVelocity = Vector3.new(2500, 2500, 2500)
				else
					Root.CFrame = OldCF
					Cleaner:Clean(ID)
				end
		...
		return "Fling", "Flinging targets"
	end,
})

Command.Add({
	Aliases = { "carpetfling", "cfling2" },
	Description = "Fling player using hip height",
	Arguments = {
		{ Name = "Player", Type = "Player" },
		{ Name = "Power", Type = "Number" },
	},
	Task = function(Players, Power)
		Power = tonumber(Power) or 1000
		local Target = Players and Players[1]
		if not Target then return end
		local OldHip = Humanoid.HipHeight
		local OldCF = Root.CFrame
		local TS = GetRoot(Target)
		if not TS then return end
		local ID = "CarpetFling"
		Cleaner:Add(ID, Connect(Services.Run.Heartbeat, function()
			Root.CFrame = TS.CFrame
		end))
		Wait()
		...
		Humanoid.Health = 0
		Connect(LocalPlayer.CharacterAdded, function(Char)
			Char:WaitForChild("HumanoidRootPart").CFrame = OldCF
		end)
		return "CarpetFling", Format("Carpet flinging %s", Target.Name)
	end,
})

Command.Add({
	Aliases = { "walkfling", "wfling" },
	Description = "Fling while walking",
	Arguments = {},
	Task = function()
		if Get("WalkFling") then
			Add("WalkFling", false)
			return "WalkFling", "Walk fling disabled"
		end
		Add("WalkFling", true)
		Spawn(function()
			local Movel = 0.1
			while Get("WalkFling") do
				Wait()
		...
		return "WalkFling", "Walk fling enabled"
	end,
})

Command.Add({
	Aliases = { "unwalkfling", "unwfling" },
	Description = "Disable walk fling",
	Arguments = {},
	Task = function()
		Add("WalkFling", false)
		return "WalkFling", "Walk fling disabled"
...
Command.Add({
	Aliases = { "fakecharacter", "fakechar", "fc2" },
	Description = "Create a fake R6 character you can control",
	Arguments = {},
	Task = function()
		local Desc = Services.Players:GetHumanoidDescriptionFromUserId(LocalPlayer.UserId)
		local NewChar = Services.Players:CreateHumanoidModelFromDescription(Desc, Enum.HumanoidRigType.R6)
		NewChar.Name = LocalPlayer.Name
		NewChar:PivotTo(Root.CFrame)
		LocalPlayer.Character = NewChar
		NewChar.Parent = workspace
		Add("FakeChar", NewChar)
		return "FakeChar", "Fake character created"
	end,
})

Command.Add({
	Aliases = { "unfakecharacter", "unfakechar", "unfc2" },
	Description = "Return to your real character",
	Arguments = {},
	Task = function()
		local Fake = Get("FakeChar")
		if Fake then
			Fake:Destroy()
			Add("FakeChar", nil)
		end
		LocalPlayer.Character = Character
		return "FakeChar", "Returned to real character"
	end,
})

Command.Add({
	Aliases = { "punchfling2" },
	Description = "Punch fling tool",
	Arguments = {},
	Task = function()
		...
		return "PunchFling", "Punch fling tool given"
	end,
})

Command.Add({
	Aliases = { "fakelag2", "teleporttoplace2", "ttp2" },
	Description = "Teleport to a place by ID",
	Arguments = {
		{ Name = "PlaceID", Type = "Number" },
	},
	Task = function(PlaceID)
		if not PlaceID then return "TpPlace", "Provide a place ID" end
		Services.Teleport:Teleport(PlaceID)
		return "TpPlace", Format("Teleporting to place %s", PlaceID)
	end,
})

Command.Add({
	Aliases = { "loophipheight" },
	Description = "Loop your hip height",
	Arguments = {
		{ Name = "Height", Type = "Number" },
	},
	Task = function(Height)
		if Get("LoopHip") then
			Add("LoopHip", false)
			return "LoopHip", "Loop hip height disabled"
		end
		Height = tonumber(Height) or 2
		Add("LoopHip", true)
		Cleaner:Add("LoopHip", Connect(Services.Run.Heartbeat, function()
			if not Get("LoopHip") then return end
			Humanoid.HipHeight = Height
		end))
		return "LoopHip", Format("Looping hip height: %s", Height)
	end,
})

Command.Add({
	Aliases = { "unloophipheight" },
	Description = "Stop loop hip height",
	Arguments = {},
	Task = function()
		Add("LoopHip", false)
		return "LoopHip", "Loop hip height disabled"
...
Command.Add({
	Aliases = { "stopdamage" },
	Description = "Prevent taking damage",
	Arguments = {},
	Task = function()
		if Get("StopDamage") then
			Add("StopDamage", false)
			return "StopDamage", "Stop damage disabled"
		end
		Add("StopDamage", true)
		Cleaner:Add("StopDamage", Connect(Humanoid:GetPropertyChangedSignal("Health"), function()
			if not Get("StopDamage") then return end
			if Humanoid.Health < Humanoid.MaxHealth then
				Humanoid.Health = Humanoid.MaxHealth
			end
		end))
		return "StopDamage", "Stop damage enabled"
	end,
})

Command.Add({
	Aliases = { "unstopdamage" },
	Description = "Allow taking damage again",
	Arguments = {},
	Task = function()
		Add("StopDamage", false)
		return "StopDamage", "Stop damage disabled"
...
Command.Add({
	...
	Task = function(Color)
		if Get("ESP") then
			Add("ESP", false)
			Cleaner:Clean("ESP")
			for _, Highlight in next, workspace:GetDescendants() do
				if Highlight:IsA("BoxHandleAdornment") and Highlight.Name == "CMD_ESP" then
					Highlight:Destroy()
				end
			end
		...
		if Color then
			...
			if R then ESPColor = Color3.fromRGB(tonumber(R), tonumber(G), tonumber(B)) end
		end
		Add("ESP", true)
		local function AddESP(Plr)
			if Plr == LocalPlayer then return end
			Cleaner:Add("ESP", Connect(Services.Run.Heartbeat, function()
				if not Get("ESP") then return end
				if not Plr.Character then return end
				for _, Part in next, Plr.Character:GetChildren() do
					if Part:IsA("BasePart") then
						local Box = Part:FindFirstChild("CMD_ESP")
						if not Box then
		...
		for _, Plr in next, Services.Players:GetPlayers() do AddESP(Plr) end
		Cleaner:Add("ESP", Connect(Services.Players.PlayerAdded, AddESP))
		return "ESP", "ESP enabled"
	end,
})
...
Command.Add({
	...
	Task = function()
		...
		Cleaner:Clean("ESP")
		for _, V in next, workspace:GetDescendants() do
			if V:IsA("BoxHandleAdornment") and V.Name == "CMD_ESP" then V:Destroy() end
		end
		return "ESP", "ESP disabled"
	end,
})
...
Command.Add({
	Aliases = { "tracers", "playertracs" },
	Description = "Draw tracers to all players",
	Arguments = {},
	Task = function()
		if Get("Tracers") then
			Add("Tracers", false)
			Cleaner:Clean("Tracers")
			for _, V in next, game:GetService("CoreGui"):GetDescendants() do
				if V.Name == "CMD_Tracer" then V:Destroy() end
			end
			return "Tracers", "Tracers disabled"
		end
		local TracerGui = Instance.new("ScreenGui")
		TracerGui.Name = "CMD_TracerGui"
		TracerGui.ResetOnSpawn = false
		TracerGui.IgnoreGuiInset = true
		TracerGui.Parent = game:GetService("CoreGui")
		Add("Tracers", true)
		Add("TracerGui", TracerGui)
		Cleaner:Add("Tracers", Connect(Services.Run.RenderStepped, function()
			if not Get("Tracers") then return end
			for _, Frame in next, TracerGui:GetChildren() do Frame:Destroy() end
			local Cam = workspace.CurrentCamera
			for _, Plr in next, Services.Players:GetPlayers() do
				if Plr == LocalPlayer then continue end
				local Root2 = GetRoot(Plr)
				if Root2 then
					local TargetPos, OnScreen = Cam:WorldToViewportPoint(Root2.Position)
					if OnScreen then
						local Line = Instance.new("Frame")
						Line.Name = "CMD_Tracer"
						Line.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
						Line.BorderSizePixel = 0
						Line.BackgroundTransparency = 0
						local ViewX, ViewY = Cam.ViewportSize.X / 2, Cam.ViewportSize.Y
						local DX = TargetPos.X - ViewX
						local DY = TargetPos.Y - ViewY
						local Dist = math.sqrt(DX * DX + DY * DY)
						Line.Size = UDim2.new(0, 1, 0, Dist)
						Line.Position = UDim2.new(0, ViewX, 0, ViewY)
		...
		return "Tracers", "Tracers enabled"
	end,
})
...
Command.Add({
	...
	Task = function()
		Add("Tracers", false)
		Cleaner:Clean("Tracers")
		local TG = Get("TracerGui")
		if TG then TG:Destroy(); Add("TracerGui", nil) end
		return "Tracers", "Tracers disabled"
	end,
})

Command.Add({
	Aliases = { "chams", "highlight" },
	...
	Task = function()
			...
			for _, Plr in next, Services.Players:GetPlayers() do
				if Plr.Character then
					for _, V in next, Plr.Character:GetDescendants() do
						if V:IsA("SelectionBox") and V.Name == "CMD_Cham" then V:Destroy() end
					end
				end
			end
			return "Chams", "Chams disabled"
		end
		Add("Chams", true)
		local function ApplyCham(Plr)
			...
			Cleaner:Add("Chams", Connect(Plr.CharacterAdded, DoChar))
		end
		for _, Plr in next, Services.Players:GetPlayers() do ApplyCham(Plr) end
		Cleaner:Add("Chams", Connect(Services.Players.PlayerAdded, ApplyCham))
		return "Chams", "Chams enabled"
	end,
})

Command.Add({
	Aliases = { "unchams", "unhighlight" },
	Description = "Disable chams",
	Arguments = {},
	Task = function()
		Add("Chams", false)
		Cleaner:Clean("Chams")
...
Command.Add({
	Aliases = { "trail" },
	Description = "Add a trail to your character",
	Arguments = {
		{ Name = "Color", Type = "String" },
	},
	Task = function(Color)
		...
		if Color then
			local R, G, B = string.match(Color, "(%d+),(%d+),(%d+)")
			if R then TrailColor = ColorSequence.new(Color3.fromRGB(tonumber(R), tonumber(G), tonumber(B))) end
		end
		...
		Trail.Parent = Root
		return "Trail", "Trail added"
	end,
})

Command.Add({
	Aliases = { "untrail" },
	Description = "Remove trail",
	Arguments = {},
	Task = function()
		for _, V in next, Root:GetChildren() do
			if V.Name == "CMD_Trail" or V.Name == "CMD_TrailA0" or V.Name == "CMD_TrailA1" then
				V:Destroy()
			end
		end
		return "Trail", "Trail removed"
	end,
})

Command.Add({
	Aliases = { "crosshair" },
	Description = "Show a crosshair on screen",
	Arguments = {},
	Task = function()
		if Get("Crosshair") then
			Add("Crosshair", false)
			local CG = Get("CrosshairGui")
			if CG then CG:Destroy(); Add("CrosshairGui", nil) end
			return "Crosshair", "Crosshair disabled"
		end
		local Gui = Instance.new("ScreenGui")
		Gui.Name = "CMD_Crosshair"
		Gui.ResetOnSpawn = false
		Gui.IgnoreGuiInset = true
		Gui.Parent = game:GetService("CoreGui")
		local function Line(W, H, X, Y)
		...
		Line(2, 20, 0, 0)
		Add("Crosshair", true)
		Add("CrosshairGui", Gui)
		return "Crosshair", "Crosshair enabled"
	end,
})
...
Command.Add({
	Aliases = { "nametag", "tag" },
	Description = "Set a custom nametag above your head",
	Arguments = {
		{ Name = "Text", Type = "String" },
	},
	Task = function(Text)
		local Head = Character:FindFirstChild("Head")
		if not Head then return "Nametag", "No head found" end
		local Old = Head:FindFirstChild("CMD_Nametag")
		if Old then Old:Destroy() end
		if not Text or Text == "" then return "Nametag", "Nametag removed" end
		...
		Label.Text = Text
		return "Nametag", Format("Nametag set to: %s", Text)
	end,
})

Command.Add({
	Aliases = { "removenametag", "untag" },
	Description = "Remove custom nametag",
	Arguments = {},
	Task = function()
		local Head = Character:FindFirstChild("Head")
		if Head then
			local Tag = Head:FindFirstChild("CMD_Nametag")
			if Tag then Tag:Destroy() end
		end
		return "Nametag", "Nametag removed"
	end,
})

Command.Add({
	Aliases = { "colorshift" },
	Description = "Color shift your character",
	Arguments = {
		{ Name = "R", Type = "Number" },
		{ Name = "G", Type = "Number" },
		{ Name = "B", Type = "Number" },
	...
	Task = function(R, G, B)
		...
		return "ColorShift", Format("Shifted color to %d,%d,%d", R, G, B)
	end,
})
...
Command.Add({
	Aliases = { "silentaim", "sa" },
	Description = "Silent aim to nearest player",
	Arguments = {},
	Task = function()
		if Get("SilentAim") then
			Add("SilentAim", false)
			return "SilentAim", "Silent aim disabled"
		end
		Add("SilentAim", true)
		return "SilentAim", "Silent aim enabled"
	end,
})

Command.Add({
	Aliases = { "unsa", "unsilentaim" },
	Description = "Disable silent aim",
	Arguments = {},
	Task = function()
		Add("SilentAim", false)
		return "SilentAim", "Silent aim disabled"
...
Command.Add({
	Aliases = { "rageaim", "raim" },
	Description = "Aim to multiple targets rapidly",
	Arguments = {
		{ Name = "Speed", Type = "Number" },
	},
	Task = function(Speed)
		if Get("RageAim") then
			Add("RageAim", false)
			return "RageAim", "Rage aim disabled"
		end
		Speed = tonumber(Speed) or 0.05
		Add("RageAim", true)
		Cleaner:Add("RageAim", Spawn(function()
			while Get("RageAim") do
				for _, Plr in next, Services.Players:GetPlayers() do
					if not Get("RageAim") then break end
					if Plr == LocalPlayer then continue end
					local Head = Plr.Character and Plr.Character:FindFirstChild("Head")
					if Head then
		...
		return "RageAim", "Rage aim enabled"
	end,
})

Command.Add({
	Aliases = { "unrageaim", "unraim" },
	Description = "Disable rage aim",
	Arguments = {},
	Task = function()
		Add("RageAim", false)
		return "RageAim", "Rage aim disabled"
...
Command.Add({
	Aliases = { "triggerbot", "tbot" },
	Description = "Auto click when hovering a player",
	Arguments = {},
	Task = function()
		if Get("TriggerBot") then
			Add("TriggerBot", false)
			return "TriggerBot", "Triggerbot disabled"
		end
		Add("TriggerBot", true)
		Cleaner:Add("TriggerBot", Connect(Services.Run.Heartbeat, function()
			if not Get("TriggerBot") then return end
			local Target = Mouse.Target
			if Target then
				local Plr = Services.Players:GetPlayerFromCharacter(Target.Parent)
				if Plr and Plr ~= LocalPlayer then
					if mouse1click then mouse1click() end
				end
			end
		end))
		return "TriggerBot", "Triggerbot enabled"
	end,
})

Command.Add({
	Aliases = { "untriggerbot", "untbot" },
	Description = "Disable triggerbot",
	Arguments = {},
	Task = function()
		Add("TriggerBot", false)
		return "TriggerBot", "Triggerbot disabled"
...
Command.Add({
	Aliases = { "instakillreach", "ikr" },
	Description = "Kill players in reach",
	Arguments = {
		{ Name = "Range", Type = "Number" },
	},
	Task = function(Range)
		if Get("InstaKillReach") then
			Add("InstaKillReach", false)
			return "InstaKillReach", "Instakill reach disabled"
		end
		Range = tonumber(Range) or 20
		Add("InstaKillReach", true)
		Cleaner:Add("InstaKillReach", Connect(Services.Run.Heartbeat, function()
			if not Get("InstaKillReach") then return end
			for _, Plr in next, Services.Players:GetPlayers() do
				if Plr == LocalPlayer then continue end
				local PlrRoot = GetRoot(Plr)
				if PlrRoot and (Root.Position - PlrRoot.Position).Magnitude <= Range then
					local PlrHum = GetHumanoid(Plr)
					if PlrHum then PlrHum.Health = 0 end
				end
			end
		end))
		return "InstaKillReach", Format("Instakill reach enabled (range: %s)", Range)
	end,
})

Command.Add({
	Aliases = { "uninstakillreach", "unikr" },
	Description = "Disable instakill reach",
	Arguments = {},
	Task = function()
		Add("InstaKillReach", false)
		return "InstaKillReach", "Instakill reach disabled"
...
Command.Add({
	Aliases = { "touchreach" },
	Description = "Kill players on touch",
	Arguments = {},
	Task = function()
		if Get("TouchReach") then
			Add("TouchReach", false)
			return "TouchReach", "Touch reach disabled"
		end
		Add("TouchReach", true)
		Cleaner:Add("TouchReach", Connect(Root.Touched, function(Part)
			if not Get("TouchReach") then return end
			local Plr = Services.Players:GetPlayerFromCharacter(Part.Parent)
			if Plr and Plr ~= LocalPlayer then
				local H = GetHumanoid(Plr)
				if H then H.Health = 0 end
			end
		end))
		return "TouchReach", "Touch reach enabled"
	end,
})

Command.Add({
	Aliases = { "untouchreach" },
	Description = "Disable touch reach",
	Arguments = {},
	Task = function()
		Add("TouchReach", false)
		return "TouchReach", "Touch reach disabled"
...
Command.Add({
	Aliases = { "aura", "killaura" },
	Description = "Kill all players in range",
	Arguments = {
		{ Name = "Range", Type = "Number" },
		{ Name = "Speed", Type = "Number" },
	},
	Task = function(Range, Speed)
		if Get("Aura") then
			Add("Aura", false)
			return "Aura", "Kill aura disabled"
		end
		Range = tonumber(Range) or 15
		Speed = tonumber(Speed) or 0.1
		Add("Aura", true)
		Cleaner:Add("Aura", Spawn(function()
			while Get("Aura") do
				for _, Plr in next, Services.Players:GetPlayers() do
					if not Get("Aura") then break end
					if Plr == LocalPlayer then continue end
					local PR = GetRoot(Plr)
					local PH = GetHumanoid(Plr)
					if PR and PH and (Root.Position - PR.Position).Magnitude <= Range then
						PH.Health = 0
					end
				end
				Wait(Speed)
		...
		return "Aura", Format("Kill aura enabled (range: %s)", Range)
	end,
})

Command.Add({
	Aliases = { "unaura", "unkillaura" },
...
Command.Add({
	Aliases = { "flingaura", "faura" },
	Description = "Fling all players in range",
	Arguments = {
		{ Name = "Range", Type = "Number" },
	},
	Task = function(Range)
		if Get("FlingAura") then
			Add("FlingAura", false)
			return "FlingAura", "Fling aura disabled"
		end
		Range = tonumber(Range) or 15
		Add("FlingAura", true)
		Cleaner:Add("FlingAura", Connect(Services.Run.Heartbeat, function()
			if not Get("FlingAura") then return end
			for _, Plr in next, Services.Players:GetPlayers() do
				if Plr == LocalPlayer then continue end
				local PR = GetRoot(Plr)
				if PR and (Root.Position - PR.Position).Magnitude <= Range then
					PR.AssemblyLinearVelocity = Vector3.new(
						math.random(-500, 500),
						math.random(200, 500),
						math.random(-500, 500)
					)
		...
		return "FlingAura", Format("Fling aura enabled (range: %s)", Range)
	end,
})

Command.Add({
	Aliases = { "unflingaura" },
	Description = "Disable fling aura",
	Arguments = {},
	Task = function()
		Add("FlingAura", false)
		return "FlingAura", "Fling aura disabled"
...
Command.Add({
	Aliases = { "ghostmode", "ghost2" },
	Description = "Make your character intangible (walk through players)",
	Arguments = {},
	Task = function()
		if Get("GhostMode") then
			Add("GhostMode", false)
			Cleaner:Clean("GhostMode")
			for _, Part in next, Character:GetDescendants() do
				if Part:IsA("BasePart") and Part.Name ~= "HumanoidRootPart" then
					Part.CanCollide = true
				end
			end
			return "GhostMode", "Ghost mode disabled"
		end
		Add("GhostMode", true)
		Cleaner:Add("GhostMode", Connect(Services.Run.Stepped, function()
			if not Get("GhostMode") then return end
			for _, Part in next, Character:GetDescendants() do
				if Part:IsA("BasePart") and Part.Name ~= "HumanoidRootPart" then
					Part.CanCollide = false
				end
		...
		return "GhostMode", "Ghost mode enabled"
	end,
})

Command.Add({
	Aliases = { "unghostmode", "unghost2" },
...
Command.Add({
	Aliases = { "invisible2", "invis2" },
	Description = "Make character invisible (v2)",
	Arguments = {},
	Task = function()
		if Get("Invisible2") then
			Add("Invisible2", false)
			for _, Part in next, Character:GetDescendants() do
				if Part:IsA("BasePart") then
					Part.LocalTransparencyModifier = 0
				end
			end
			return "Invisible2", "Invisible disabled"
		end
		Add("Invisible2", true)
		Cleaner:Add("Invisible2", Connect(Services.Run.RenderStepped, function()
			if not Get("Invisible2") then return end
			for _, Part in next, Character:GetDescendants() do
				if Part:IsA("BasePart") then
		...
		return "Invisible2", "Invisible enabled"
	end,
})

-- ============================
-- BHOP / MOVEMENT TRICKS
-- ============================

Command.Add({
	Aliases = { "bhop", "bunnyhop" },
	...
	Task = function(Power)
		if Get("Bhop") then
			Add("Bhop", false)
			return "Bhop", "Bhop disabled"
		end
		Power = tonumber(Power) or 60
		Add("Bhop", true)
		Cleaner:Add("Bhop", Connect(Services.Run.Heartbeat, function()
			if not Get("Bhop") then return end
			if Humanoid:GetState() == Enum.HumanoidStateType.Landed
				or Humanoid:GetState() == Enum.HumanoidStateType.Running then
				Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
			end
		end))
		return "Bhop", Format("Bhop enabled (power: %s)", Power)
	end,
})

Command.Add({
	Aliases = { "unbhop", "unbunnyhop" },
	Description = "Disable bhop",
	Arguments = {},
	Task = function()
		Add("Bhop", false)
		return "Bhop", "Bhop disabled"
...
Command.Add({
	Aliases = { "supermanfly", "superman" },
	Description = "Fly without tools",
	Arguments = {
		{ Name = "Speed", Type = "Number" },
	},
	Task = function(Speed)
		if Get("SuperFly") then
			Add("SuperFly", false)
			Cleaner:Clean("SuperFly")
			Humanoid.PlatformStand = false
			return "SuperFly", "Superman fly disabled"
		end
		...
		Add("SuperFlyBV", BV)
		Add("SuperFlyBG", BG)
		Cleaner:Add("SuperFly", Connect(Services.Run.RenderStepped, function()
			if not Get("SuperFly") then return end
			BG.CFrame = Camera.CFrame
			local Dir = Vector3.zero
			if Services.Input:IsKeyDown(Enum.KeyCode.W) then Dir = Dir + Camera.CFrame.LookVector end
			if Services.Input:IsKeyDown(Enum.KeyCode.S) then Dir = Dir - Camera.CFrame.LookVector end
			if Services.Input:IsKeyDown(Enum.KeyCode.A) then Dir = Dir - Camera.CFrame.RightVector end
			if Services.Input:IsKeyDown(Enum.KeyCode.D) then Dir = Dir + Camera.CFrame.RightVector end
			if Services.Input:IsKeyDown(Enum.KeyCode.E) then Dir = Dir + Vector3.new(0, 1, 0) end
			if Services.Input:IsKeyDown(Enum.KeyCode.Q) then Dir = Dir - Vector3.new(0, 1, 0) end
			BV.Velocity = Dir.Magnitude > 0 and Dir.Unit * Speed or Vector3.zero
		end))
		return "SuperFly", Format("Superman fly enabled (speed: %s)", Speed)
	end,
})

Command.Add({
	Aliases = { "unsupermanfly", "unsuperman" },
	Description = "Disable superman fly",
	Arguments = {},
	Task = function()
		Add("SuperFly", false)
		Cleaner:Clean("SuperFly")
		local BV = Get("SuperFlyBV")
		local BG = Get("SuperFlyBG")
...
Command.Add({
	Aliases = { "betterfly2", "bfly2" },
	Description = "Smooth fly mode with WASD",
	Arguments = {
		{ Name = "Speed", Type = "Number" },
	},
	Task = function(Speed)
		if Get("BetterFly") then
			Add("BetterFly", false)
			Cleaner:Clean("BetterFly")
			SetFly(false)
			Humanoid.WalkSpeed = 16
			return "BetterFly", "Better fly disabled"
		end
		Speed = tonumber(Speed) or 50
		SetFly(true, Speed / 16)
		Add("BetterFly", true)
		return "BetterFly", Format("Better fly enabled (speed: %s)", Speed)
	end,
})

Command.Add({
	Aliases = { "unbetterfly2", "unbfly2" },
	Description = "Disable better fly",
	Arguments = {},
	Task = function()
		Add("BetterFly", false)
		Cleaner:Clean("BetterFly")
...
Command.Add({
	Aliases = { "partcontrol", "pc" },
	Description = "Control parts with your character",
	Arguments = {
		{ Name = "Part", Type = "String" },
	},
	Task = function(PartName)
		if Get("PartControl") then
			Add("PartControl", false)
			Cleaner:Clean("PartControl")
			return "PartControl", "Part control disabled"
		end
		local TargetPart = PartName and workspace:FindFirstChild(PartName)
			or Mouse.Target
		if not TargetPart then return "PartControl", "No part found" end
		Add("PartControl", true)
		Add("PartControlTarget", TargetPart)
		Cleaner:Add("PartControl", Connect(Services.Run.Heartbeat, function()
			if not Get("PartControl") then return end
			local T = Get("PartControlTarget")
			if T then T.CFrame = Root.CFrame end
		end))
		return "PartControl", Format("Controlling: %s", TargetPart.Name)
	end,
})

Command.Add({
	Aliases = { "unpartcontrol", "unpc" },
	Description = "Stop part control",
	Arguments = {},
	Task = function()
		Add("PartControl", false)
		Cleaner:Clean("PartControl")
		Add("PartControlTarget", nil)
		return "PartControl", "Part control disabled"
	end,
})

Command.Add({
	Aliases = { "partstorm", "ps" },
	Description = "Spawn a storm of parts around you",
	Arguments = {
		{ Name = "Count", Type = "Number" },
		{ Name = "Size", Type = "Number" },
	},
	Task = function(Count, Size)
		if Get("PartStorm") then
			Add("PartStorm", false)
			Cleaner:Clean("PartStorm")
			for _, V in next, workspace:GetChildren() do
				if V.Name == "CMD_StormPart" then V:Destroy() end
			end
			return "PartStorm", "Part storm disabled"
		end
		Count = math.clamp(tonumber(Count) or 20, 1, 50)
		Size = tonumber(Size) or 2
		Add("PartStorm", true)
		for i = 1, Count do
			local Part = Instance.new("Part")
			Part.Name = "CMD_StormPart"
			Part.Size = Vector3.new(Size, Size, Size)
			Part.Material = Enum.Material.Neon
			Part.BrickColor = BrickColor.Random()
			Part.CFrame = Root.CFrame * CFrame.new(
				math.random(-10, 10), math.random(-5, 5), math.random(-10, 10)
			)
			Part.Parent = workspace
		end
		Cleaner:Add("PartStorm", Connect(Services.Run.Heartbeat, function()
			if not Get("PartStorm") then return end
			local Angle = os.clock() * 3
			local Idx = 0
			for _, Part in next, workspace:GetChildren() do
				if Part.Name == "CMD_StormPart" then
					Idx = Idx + 1
					local A = Angle + (Idx / Count) * math.pi * 2
					Part.CFrame = CFrame.new(
						Root.Position + Vector3.new(math.cos(A) * 8, math.sin(Idx) * 3, math.sin(A) * 8)
					)
				end
			end
		end))
		return "PartStorm", Format("Part storm enabled (%d parts)", Count)
	end,
})

Command.Add({
	Aliases = { "unpartstorm" },
	Description = "Disable part storm",
	Arguments = {},
	Task = function()
		Add("PartStorm", false)
		Cleaner:Clean("PartStorm")
...
Command.Add({
	Aliases = { "parttrap", "ptrap" },
	Description = "Trap a player with parts",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		local Target = Players and Players[1]
		if not Target then return "PartTrap", "No player found" end
		local TR = GetRoot(Target)
		if not TR then return "PartTrap", "Target has no root" end
		local Offsets = {
			Vector3.new(3.5, 0, 0), Vector3.new(-3.5, 0, 0),
			Vector3.new(0, 0, 3.5), Vector3.new(0, 0, -3.5),
			Vector3.new(0, 4, 0), Vector3.new(0, -1, 0),
		}
		for _, Offset in next, Offsets do
			...
			Part.Anchored = true
			Part.CFrame = CFrame.new(TR.Position + Offset)
			Part.Transparency = 0.5
			Part.BrickColor = BrickColor.new("Bright red")
			Part.Parent = workspace
		end
		return "PartTrap", Format("Trapped %s", Target.Name)
	end,
})

Command.Add({
	Aliases = { "cleartraps", "removetrap" },
	Description = "Remove all traps",
	Arguments = {},
	Task = function()
		local Count = 0
		for _, V in next, workspace:GetDescendants() do
			if V.Name == "CMD_Trap" then V:Destroy(); Count += 1 end
		end
		return "ClearTraps", Format("Removed %d trap(s)", Count)
	end,
})

Command.Add({
	Aliases = { "blackhole", "bh2" },
	Description = "Create a black hole that sucks players",
	Arguments = {
		{ Name = "Power", Type = "Number" },
		{ Name = "Range", Type = "Number" },
	},
	Task = function(Power, Range)
		if Get("Blackhole") then
			Add("Blackhole", false)
			Cleaner:Clean("Blackhole")
			return "Blackhole", "Blackhole disabled"
		end
		Power = tonumber(Power) or 200
		Range = tonumber(Range) or 50
		Add("Blackhole", true)
		local Position = Root.Position
		Cleaner:Add("Blackhole", Connect(Services.Run.Heartbeat, function()
			if not Get("Blackhole") then return end
			for _, Plr in next, Services.Players:GetPlayers() do
				if Plr == LocalPlayer then continue end
				local PR = GetRoot(Plr)
				if PR then
					local Dist = (Position - PR.Position).Magnitude
					if Dist <= Range then
						local Dir = (Position - PR.Position).Unit
						PR.AssemblyLinearVelocity = Dir * Power
					end
				end
			end
		end))
		return "Blackhole", Format("Blackhole active (power: %s, range: %s)", Power, Range)
	end,
})

Command.Add({
	Aliases = { "unblackhole" },
	Description = "Disable blackhole",
	Arguments = {},
	Task = function()
		Add("Blackhole", false)
		Cleaner:Clean("Blackhole")
...
Command.Add({
	Aliases = { "addalias2" },
	Description = "Add alias for a command",
	Arguments = {
		{ Name = "Command", Type = "String" },
		{ Name = "Alias", Type = "String" },
	},
	Task = function(CmdName, Alias)
		if not CmdName or not Alias then return "AddAlias", "Provide command and alias" end
		local Found = Command.Find(CmdName)
		if Found then
			table.insert(Found[1], Alias)
			return "AddAlias", Format("Added alias '%s' for '%s'", Alias, CmdName)
		end
		return "AddAlias", Format("Command '%s' not found", CmdName)
	end,
})

Command.Add({
	Aliases = { "aliases2" },
	Description = "Show all aliases of a command",
	Arguments = {
		{ Name = "Command", Type = "String" },
	},
	Task = function(CmdName)
		if not CmdName then return "Aliases", "Provide a command name" end
		local Found = Command.Find(CmdName)
		if Found then
			return "Aliases", Format("%s: %s", CmdName, table.concat(Found[1], ", "))
		end
		return "Aliases", Format("Command '%s' not found", CmdName)
	end,
})
...
Command.Add({
	...
	Task = function(Amount)
		...
		return "MaxHealth", Format("Max health set to %s", Amount)
	end,
})

Command.Add({
	Aliases = { "sethealth", "hp" },
	...
	Task = function(Amount)
		Amount = tonumber(Amount) or 100
		Humanoid.Health = Amount
		return "SetHealth", Format("Health set to %s", Amount)
	end,
})

Command.Add({
	Aliases = { "heal", "fullheal" },
	Description = "Heal to full health",
	Arguments = {},
	Task = function()
		Humanoid.Health = Humanoid.MaxHealth
		return "Heal", "Healed to full health"
	end,
})

Command.Add({
	Aliases = { "bringtome", "bring2" },
	...
	Task = function(Players)
		for _, Player in next, Players do
			local TR = GetRoot(Player)
			if TR then
				TR.CFrame = Root.CFrame * CFrame.new(0, 0, -3)
			end
		end
		return "BringToMe", "Players brought to you"
	end,
})

Command.Add({
	Aliases = { "seatbring2" },
	Description = "Teleport to a player via seat",
	Arguments = {
		{ Name = "Player", Type = "Player" },
	},
	Task = function(Players)
		...
		Seat.Transparency = 1
		Seat.CFrame = TR.CFrame
		Seat.Parent = workspace
		Seat:Sit(Humanoid)
		Wait(0.5)
		Seat:Destroy()
		return "SeatBring", Format("Teleported to %s via seat", Target.Name)
	end,
})
...
Command.Add({
	...
	Task = function()
		if Get("ClickTP") then
			Add("ClickTP", false)
			return "ClickTP", "Click TP disabled"
		end
		Add("ClickTP", true)
		Cleaner:Add("ClickTP", Connect(Mouse.Button1Down, function()
			if not Get("ClickTP") then return end
			local Target = Mouse.Hit
			if Target then
				Root.CFrame = CFrame.new(Target.Position + Vector3.new(0, 3, 0))
			end
		end))
		return "ClickTP", "Click TP enabled (click to teleport)"
	end,
})

Command.Add({
	Aliases = { "unclicktp2" },
	Description = "Disable click TP",
	Arguments = {},
	Task = function()
		Add("ClickTP", false)
		return "ClickTP", "Click TP disabled"
...
Command.Add({
	Aliases = { "walkto2", "moveto" },
	Description = "Move character to a position",
	Arguments = {
		{ Name = "X", Type = "Number" },
		{ Name = "Y", Type = "Number" },
		{ Name = "Z", Type = "Number" },
	...
	Task = function(X, Y, Z)
		...
		return "WalkTo", Format("Walking to %s, %s, %s", X, Y, Z)
	end,
})

Command.Add({
	Aliases = { "stopwalkto2" },
	Description = "Stop walking to position",
	Arguments = {},
	Task = function()
		Humanoid:MoveTo(Root.Position)
		return "StopWalkTo", "Stopped walking"
	end,
})

Command.Add({
	Aliases = { "grabtp2" },
	Description = "TP to every player rapidly (grab TP)",
	Arguments = {
		{ Name = "Speed", Type = "Number" },
	},
	Task = function(Speed)
		if Get("GrabTP") then
			Add("GrabTP", false)
			return "GrabTP", "Grab TP disabled"
		end
		Speed = tonumber(Speed) or 0.1
		Add("GrabTP", true)
		Cleaner:Add("GrabTP", Spawn(function()
			while Get("GrabTP") do
				for _, Plr in next, Services.Players:GetPlayers() do
					if not Get("GrabTP") then break end
					if Plr == LocalPlayer then continue end
					local TR = GetRoot(Plr)
					if TR then Root.CFrame = TR.CFrame end
				end
				Wait(Speed)
			end
		end))
		return "GrabTP", Format("Grab TP enabled (speed: %ss)", Speed)
	end,
})

Command.Add({
	Aliases = { "ungrabtp2" },
	Description = "Disable grab TP",
	Arguments = {},
	Task = function()
		Add("GrabTP", false)
		return "GrabTP", "Grab TP disabled"
...
Command.Add({
	Aliases = { "vehiclecontrol", "vc" },
	Description = "Control a vehicle part",
	Arguments = {},
	Task = function()
		if Get("VehicleControl") then
			Add("VehicleControl", false)
			return "VehicleControl", "Vehicle control disabled"
		end
		local Target = Mouse.Target
		if not Target then return "VehicleControl", "Click a part first" end
		Add("VehicleControl", true)
		Add("VehicleControlPart", Target)
		Cleaner:Add("VehicleControl", Connect(Services.Run.Heartbeat, function()
			if not Get("VehicleControl") then return end
			local Part = Get("VehicleControlPart")
			if Part and Part.Parent then
				Part.CFrame = Root.CFrame
			end
		end))
		return "VehicleControl", Format("Controlling: %s", Target.Name)
	end,
})

Command.Add({
	Aliases = { "unvehiclecontrol", "unvc" },
	Description = "Stop vehicle control",
	Arguments = {},
	Task = function()
		Add("VehicleControl", false)
		Add("VehicleControlPart", nil)
		return "VehicleControl", "Vehicle control disabled"
	end,
})

Command.Add({
	Aliases = { "interact", "touchpart" },
	Description = "Fire touch on a part",
	Arguments = {},
	Task = function()
		if not firetouchinterest then
			return "Interact", "firetouchinterest not available"
		end
		local Target = Mouse.Target
		if not Target then return "Interact", "No target found" end
		firetouchinterest(Target, Root, 0)
		firetouchinterest(Target, Root, 1)
		return "Interact", Format("Touched: %s", Target.Name)
	end,
})
...
Command.Add({
	...
	Task = function(Amount)
		Amount = math.clamp(tonumber(Amount) or 70, 1, 120)
		Camera.FieldOfView = Amount
		return "FOV", Format("FOV set to %s", Amount)
	end,
})

Command.Add({
	Aliases = { "showfov" },
	Description = "Show a FOV circle",
	Arguments = {
		{ Name = "Radius", Type = "Number" },
	},
	Task = function(Radius)
		if Get("ShowFOV") then
			Add("ShowFOV", false)
			local FG = Get("FOVGui")
			if FG then FG:Destroy() end
			return "ShowFOV", "FOV circle hidden"
		end
		Radius = tonumber(Radius) or 100
		local Gui = Instance.new("ScreenGui")
		Gui.Name = "CMD_FOVCircle"
		Gui.ResetOnSpawn = false
		Gui.IgnoreGuiInset = true
		Gui.Parent = game:GetService("CoreGui")
		...
		Add("ShowFOV", true)
		Add("FOVGui", Gui)
		return "ShowFOV", Format("FOV circle shown (radius: %s)", Radius)
	end,
})

Command.Add({
	Aliases = { "hidecoreguixxx", "hidegui" },
	Description = "Hide a CoreGui element",
	Arguments = {
		{ Name = "Name", Type = "String" },
	},
	Task = function(Name)
		if not Name then return "HideGui", "Provide a GUI name" end
		local CGS = game:GetService("CoreGui")
		local Target = CGS:FindFirstChild(Name)
		if Target then
			Target.Enabled = false
			return "HideGui", Format("Hidden: %s", Name)
		end
		return "HideGui", Format("Not found: %s", Name)
	end,
})

Command.Add({
	Aliases = { "enablecoreuis2", "showgui" },
	Description = "Enable CoreGui by name",
	Arguments = {
		{ Name = "Name", Type = "String" },
	},
	Task = function(Name)
		if not Name then return "ShowGui", "Provide a GUI name" end
		local CGS = game:GetService("CoreGui")
		local Target = CGS:FindFirstChild(Name)
		if Target then
			Target.Enabled = true
			return "ShowGui", Format("Enabled: %s", Name)
		end
		return "ShowGui", Format("Not found: %s", Name)
	end,
})

Command.Add({
	Aliases = { "logmodules", "modules" },
	Description = "List all required modules in the game",
	Arguments = {},
	Task = function()
		local Mods = {}
		...
		return "LogModules", Format("Logged %d modules (see console)", #Mods)
	end,
})

Command.Add({
	Aliases = { "ping2", "nping" },
	Description = "Show numeric ping in studs per second delay",
	Arguments = {},
	Task = function()
		local Ping = math.floor(LocalPlayer:GetNetworkPing() * 1000)
		if Ping < 100 then
			return "Ping", Format("Good ping: %dms", Ping)
		elseif Ping < 200 then
			return "Ping", Format("Average ping: %dms", Ping)
		else
			return "Ping", Format("Bad ping: %dms", Ping)
		end
	end,
})

Command.Add({
	Aliases = { "reloadnetwork2", "rnet" },
	Description = "Reload network settings",
	Arguments = {},
	Task = function()
		local NS = settings():GetService("NetworkSettings")
		NS.IncomingReplicationLag = 0
		return "ReloadNetwork", "Network reloaded"
	end,
})

Command.Add({
	Aliases = { "hidefov2", "unhidecrosshair" },
	Description = "Hide the FOV circle",
	Arguments = {},
	Task = function()
		Add("ShowFOV", false)
		local FG = Get("FOVGui")
		if FG then FG:Destroy(); Add("FOVGui", nil) end
		return "ShowFOV", "FOV circle hidden"
	end,
})
...
Command.Add({
	Aliases = { "illusion" },
	Description = "Create an illusion of yourself at your position",
	Arguments = {},
	Task = function()
		Character.Archivable = true
		local Clone = Character:Clone()
		...
		Clone.Parent = workspace
		Add("IllusionClone", Clone)
		return "Illusion", "Illusion created"
	end,
})

Command.Add({
	Aliases = { "unillusion" },
	Description = "Remove the illusion",
	Arguments = {},
	Task = function()
		local Clone = Get("IllusionClone")
		if Clone then Clone:Destroy(); Add("IllusionClone", nil) end
		return "Illusion", "Illusion removed"
	end,
})

Command.Add({
	Aliases = { "hitsound" },
	Description = "Play a sound on kill",
	Arguments = {
		{ Name = "SoundID", Type = "String" },
	},
	Task = function(SoundID)
		if Get("HitSound") then
			Add("HitSound", false)
			Cleaner:Clean("HitSound")
			return "HitSound", "Hitsound disabled"
		end
		SoundID = SoundID or "131961136"
		Add("HitSound", true)
		local Sound = Instance.new("Sound")
		Sound.SoundId = "rbxassetid://" .. SoundID
		Sound.Volume = 1
		Sound.Parent = game:GetService("SoundService")
		Add("HitSoundObj", Sound)
		for _, Plr in next, Services.Players:GetPlayers() do
			if Plr == LocalPlayer then continue end
			Cleaner:Add("HitSound", Connect(Plr.CharacterAdded, function()
				local H = GetHumanoid(Plr)
				if H then
					Cleaner:Add("HitSound", Connect(H.Died, function()
						Sound:Play()
					end))
				end
			end))
		end
		return "HitSound", Format("Hitsound enabled (ID: %s)", SoundID)
	end,
})

Command.Add({
	Aliases = { "unhitsound" },
	Description = "Disable hitsound",
	Arguments = {},
	Task = function()
		Add("HitSound", false)
		Cleaner:Clean("HitSound")
		local S = Get("HitSoundObj")
		if S then S:Destroy(); Add("HitSoundObj", nil) end
		return "HitSound", "Hitsound disabled"
	end,
})
...
Command.Add({
	Aliases = { "mousefling2" },
	Description = "Fling using mouse position",
	Arguments = {
		{ Name = "Power", Type = "Number" },
	},
	Task = function(Power)
		if Get("MouseFling") then
			Add("MouseFling", false)
			return "MouseFling", "Mouse fling disabled"
		end
		Power = tonumber(Power) or 500
		Add("MouseFling", true)
		Cleaner:Add("MouseFling", Connect(Services.Run.Heartbeat, function()
			if not Get("MouseFling") then return end
			local Target = Mouse.Target
			if not Target then return end
			local Plr = Services.Players:GetPlayerFromCharacter(Target.Parent)
			if Plr and Plr ~= LocalPlayer then
				...
				if PR then
					...
					PR.AssemblyLinearVelocity = Dir * Power + Vector3.new(0, Power / 2, 0)
				end
			end
		end))
		return "MouseFling", Format("Mouse fling enabled (power: %s)", Power)
	end,
})

Command.Add({
	Aliases = { "unmousefling2" },
	Description = "Disable mouse fling",
	Arguments = {},
	Task = function()
		Add("MouseFling", false)
		return "MouseFling", "Mouse fling disabled"
...
Command.Add({
	Aliases = { "controlnpc", "cnpc" },
	Description = "Control a nearby NPC",
	Arguments = {},
	Task = function()
		if Get("ControlNPC") then
			Add("ControlNPC", false)
			Cleaner:Clean("ControlNPC")
			if LocalPlayer.Character ~= Character then
				LocalPlayer.Character = Character
			end
			return "ControlNPC", "NPC control disabled"
		end
		local Closest, ClosestDist = nil, math.huge
		for _, Model in next, workspace:GetDescendants() do
			if Model:IsA("Model") and Model:FindFirstChildOfClass("Humanoid") then
				if not Services.Players:GetPlayerFromCharacter(Model) and Model ~= Character then
					local NRoot = Model:FindFirstChild("HumanoidRootPart")
					if NRoot then
						local Dist = (Root.Position - NRoot.Position).Magnitude
		...
		if not Closest then return "ControlNPC", "No NPC found nearby" end
		Add("ControlNPC", true)
		LocalPlayer.Character = Closest
		return "ControlNPC", Format("Controlling NPC: %s", Closest.Name)
	end,
})

Command.Add({
	Aliases = { "uncontrolnpc", "uncnpc" },
	Description = "Stop controlling NPC",
	Arguments = {},
	Task = function()
		Add("ControlNPC", false)
		Cleaner:Clean("ControlNPC")
		LocalPlayer.Character = Character
		return "ControlNPC", "Returned to own character"
	end,
})

Command.Add({
	Aliases = { "bringnpc", "bnpc" },
	Description = "Bring nearest NPC to you",
	Arguments = {
		{ Name = "Count", Type = "Number" },
	},
	Task = function(Count)
		...
		for _, Model in next, workspace:GetDescendants() do
			...
			if Model:IsA("Model") and Model:FindFirstChildOfClass("Humanoid") then
				if not Services.Players:GetPlayerFromCharacter(Model) then
					local NRoot = Model:FindFirstChild("HumanoidRootPart")
					if NRoot then
						NRoot.CFrame = Root.CFrame * CFrame.new(0, 0, -4 - Found)
						Found += 1
					end
				end
			end
		end
		return "BringNPC", Format("Brought %d NPC(s)", Found)
	end,
})

Command.Add({
	Aliases = { "killnpc", "knpc" },
	Description = "Kill all NPCs",
	Arguments = {},
	Task = function()
		local Count = 0
		for _, Model in next, workspace:GetDescendants() do
			if Model:IsA("Model") then
				...
				if H and not Services.Players:GetPlayerFromCharacter(Model) then
					H.Health = 0
					Count += 1
				end
			end
		end
		return "KillNPC", Format("Killed %d NPC(s)", Count)
	end,
})

Command.Add({
	Aliases = { "flingnpc", "fnpc" },
	Description = "Fling all NPCs",
	Arguments = {
		{ Name = "Power", Type = "Number" },
	},
	Task = function(Power)
		...
		for _, Model in next, workspace:GetDescendants() do
			if Model:IsA("Model") then
				local NRoot = Model:FindFirstChild("HumanoidRootPart")
				local H = Model:FindFirstChildOfClass("Humanoid")
				if NRoot and H and not Services.Players:GetPlayerFromCharacter(Model) then
					NRoot.AssemblyLinearVelocity = Vector3.new(
		...
		return "FlingNPC", Format("Flung %d NPC(s)", Count)
	end,
})

Command.Add({
	Aliases = { "voidnpc", "vnpc" },
	Description = "Send all NPCs to the void",
	Arguments = {},
	Task = function()
		local Count = 0
		for _, Model in next, workspace:GetDescendants() do
			if Model:IsA("Model") then
				...
				local H = Model:FindFirstChildOfClass("Humanoid")
				if NRoot and H and not Services.Players:GetPlayerFromCharacter(Model) then
					NRoot.CFrame = CFrame.new(0, -10000, 0)
					Count += 1
				end
			end
		end
		return "VoidNPC", Format("Voided %d NPC(s)", Count)
	end,
})

Command.Add({
	Aliases = { "clickflingnpc", "cfnpc2" },
	Description = "Fling NPC you click on",
	Arguments = {
		{ Name = "Power", Type = "Number" },
	},
	Task = function(Power)
		if Get("ClickFlingNPC") then
			Add("ClickFlingNPC", false)
			Cleaner:Clean("ClickFlingNPC")
			return "ClickFlingNPC", "Click fling NPC disabled"
		end
		Power = tonumber(Power) or 1000
		Add("ClickFlingNPC", true)
		Cleaner:Add("ClickFlingNPC", Connect(Mouse.Button1Down, function()
			if not Get("ClickFlingNPC") then return end
			local Target = Mouse.Target
			if not Target then return end
			local Model = Target:FindFirstAncestorOfClass("Model")
			if Model then
				...
				if H and NR and not Services.Players:GetPlayerFromCharacter(Model) then
					NR.AssemblyLinearVelocity = Vector3.new(
						math.random(-Power, Power), Power,
						math.random(-Power, Power)
					)
				end
			end
		end))
		return "ClickFlingNPC", Format("Click fling NPC enabled (power: %s)", Power)
	end,
})
...
Command.Add({
	Aliases = { "antiteleport", "anttp" },
	Description = "Prevent being teleported",
	Arguments = {},
	Task = function()
		if Get("AntiTP") then
			Add("AntiTP", false)
			Cleaner:Clean("AntiTP")
			return "AntiTP", "Anti-teleport disabled"
		end
		Add("AntiTP", true)
		local LastPos = Root.CFrame
		Cleaner:Add("AntiTP", Connect(Services.Run.Heartbeat, function()
			if not Get("AntiTP") then return end
			local Diff = (Root.Position - LastPos.Position).Magnitude
			if Diff > 50 then
				Root.CFrame = LastPos
			else
				LastPos = Root.CFrame
		...
		return "AntiTP", "Anti-teleport enabled"
	end,
})

Command.Add({
	Aliases = { "unantiteleport", "unanttp" },
	Description = "Disable anti-teleport",
	Arguments = {},
	Task = function()
		Add("AntiTP", false)
		Cleaner:Clean("AntiTP")
...
Command.Add({
	Aliases = { "antipause2", "antilag" },
	Description = "Prevent pausing",
	Arguments = {},
	Task = function()
		if Get("AntiPause") then
			Add("AntiPause", false)
			return "AntiPause", "Anti-pause disabled"
		end
		Add("AntiPause", true)
		Cleaner:Add("AntiPause", Connect(game:GetService("GuiService").MenuOpened, function()
			if not Get("AntiPause") then return end
			game:GetService("GuiService"):SetMenuOpen(false)
		end))
		return "AntiPause", "Anti-pause enabled"
	end,
})

Command.Add({
	Aliases = { "antiknockback2", "antikb2" },
	Description = "Prevent being knocked back",
	Arguments = {},
	Task = function()
		if Get("AntiKB") then
			Add("AntiKB", false)
			Cleaner:Clean("AntiKB")
			return "AntiKB", "Anti-knockback disabled"
		end
		Add("AntiKB", true)
		local SavedVel = Vector3.zero
		Cleaner:Add("AntiKB", Connect(Services.Run.Heartbeat, function()
			if not Get("AntiKB") then return end
			local Vel = Root.AssemblyLinearVelocity
			if Vel.Magnitude > 100 then
		...
		return "AntiKB", "Anti-knockback enabled"
	end,
})

Command.Add({
	Aliases = { "unantikb2" },
	Description = "Disable anti-knockback",
	Arguments = {},
	Task = function()
		Add("AntiKB", false)
		Cleaner:Clean("AntiKB")
...
Command.Add({
	Aliases = { "antikill2", "antideath" },
	Description = "Prevent being killed",
	Arguments = {},
	Task = function()
		if Get("AntiKill") then
			Add("AntiKill", false)
			Cleaner:Clean("AntiKill")
			return "AntiKill", "Anti-kill disabled"
		end
		Add("AntiKill", true)
		Cleaner:Add("AntiKill", Connect(Humanoid.Died, function()
			if not Get("AntiKill") then return end
			Humanoid.Health = Humanoid.MaxHealth
		end))
		Cleaner:Add("AntiKill", Connect(Services.Run.Heartbeat, function()
			if not Get("AntiKill") then return end
			if Humanoid.Health <= 0 then
				Humanoid.Health = Humanoid.MaxHealth
			end
		end))
		return "AntiKill", "Anti-kill enabled"
	end,
})

Command.Add({
	Aliases = { "unantikill2" },
	Description = "Disable anti-kill",
	Arguments = {},
	Task = function()
		Add("AntiKill", false)
		Cleaner:Clean("AntiKill")
...
Command.Add({
	Aliases = { "antisit2", "nosit2" },
	Description = "Prevent sitting in seats",
	Arguments = {},
	Task = function()
		if Get("AntiSit") then
			Add("AntiSit", false)
			Cleaner:Clean("AntiSit")
			return "AntiSit", "Anti-sit disabled"
		end
		Add("AntiSit", true)
		Cleaner:Add("AntiSit", Connect(Humanoid:GetPropertyChangedSignal("Sit"), function()
			if not Get("AntiSit") then return end
			if Humanoid.Sit then
				Humanoid.Sit = false
			end
		end))
		return "AntiSit", "Anti-sit enabled"
	end,
})
...
Command.Add({
	Aliases = { "anticframeteleport2", "anticframe" },
	Description = "Prevent CFrame teleport exploits",
	Arguments = {},
	Task = function()
		if Get("AntiCFrame") then
			Add("AntiCFrame", false)
			Cleaner:Clean("AntiCFrame")
			return "AntiCFrame", "Anti-CFrame teleport disabled"
		end
		Add("AntiCFrame", true)
		local LastCFrame = Root.CFrame
		Cleaner:Add("AntiCFrame", Connect(Services.Run.Heartbeat, function()
			if not Get("AntiCFrame") then return end
			if (Root.Position - LastCFrame.Position).Magnitude > 80 then
				Root.CFrame = LastCFrame
			else
				LastCFrame = Root.CFrame
			end
		end))
		return "AntiCFrame", "Anti-CFrame teleport enabled"
	end,
})
...
Command.Add({
	Aliases = { "emote2", "emotion" },
	Description = "Play an emote by ID",
	Arguments = {
		{ Name = "ID", Type = "String" },
	},
	Task = function(ID)
		if not ID then return "Emote", "Provide an emote ID" end
		local Animator = Humanoid:FindFirstChildOfClass("Animator")
			or Instance.new("Animator", Humanoid)
		local Anim = Instance.new("Animation")
		Anim.AnimationId = "rbxassetid://" .. ID
		Animator:LoadAnimation(Anim):Play()
		return "Emote", Format("Playing emote %s", ID)
	end,
})

Command.Add({
	Aliases = { "dance2", "danceloop" },
	Description = "Loop a dance animation",
	Arguments = {
		{ Name = "ID", Type = "String" },
	},
	Task = function(ID)
		if Get("Dance") then
			Add("Dance", false)
			local Track = Get("DanceTrack")
			if Track then Track:Stop() end
			return "Dance", "Dance stopped"
		end
		if not ID then return "Dance", "Provide animation ID" end
		local Animator = Humanoid:FindFirstChildOfClass("Animator")
			or Instance.new("Animator", Humanoid)
		local Anim = Instance.new("Animation")
		Anim.AnimationId = "rbxassetid://" .. ID
		...
		Track:Play()
		Add("Dance", true)
		Add("DanceTrack", Track)
		return "Dance", Format("Dancing with anim %s", ID)
	end,
})

Command.Add({
	Aliases = { "stopdance", "undance2" },
...
Command.Add({
	Aliases = { "partwalkfling", "pwf" },
	Description = "Fling when touching parts",
	Arguments = {},
	Task = function()
		if Get("PartWalkFling") then
			Add("PartWalkFling", false)
			Cleaner:Clean("PartWalkFling")
			return "PartWalkFling", "Part walk fling disabled"
		end
		Add("PartWalkFling", true)
		local Touched = {}
		Cleaner:Add("PartWalkFling", Connect(Root.Touched, function(Part)
			if not Get("PartWalkFling") then return end
			if Touched[Part] then return end
			local Plr = Services.Players:GetPlayerFromCharacter(Part.Parent)
			if Plr then return end
			Touched[Part] = true
			Part.AssemblyLinearVelocity = Part.AssemblyLinearVelocity
				+ Vector3.new(math.random(-500, 500), 500, math.random(-500, 500))
			task.delay(0.5, function() Touched[Part] = nil end)
		end))
		return "PartWalkFling", "Part walk fling enabled"
	end,
})

Command.Add({
	Aliases = { "unpartwalkfling", "unpwf" },
	Description = "Disable part walk fling",
	Arguments = {},
	Task = function()
		Add("PartWalkFling", false)
		Cleaner:Clean("PartWalkFling")
...
Command.Add({
	Aliases = { "uafling", "uaf" },
	Description = "Fling unanchored parts",
	Arguments = {
		{ Name = "Power", Type = "Number" },
		{ Name = "Range", Type = "Number" },
	},
	Task = function(Power, Range)
		...
		for _, V in next, workspace:GetDescendants() do
			if V:IsA("BasePart") and not V.Anchored
				and (Root.Position - V.Position).Magnitude <= Range then
				if not Services.Players:GetPlayerFromCharacter(V.Parent) then
		...
		return "UAFling", Format("Flung %d unanchored parts", Count)
	end,
})

Command.Add({
	Aliases = { "spamjump", "jumpspam" },
	Description = "Spam jump",
	Arguments = {},
	Task = function()
		if Get("SpamJump") then
			Add("SpamJump", false)
			return "SpamJump", "Spam jump disabled"
		end
		Add("SpamJump", true)
		Cleaner:Add("SpamJump", Connect(Services.Run.Heartbeat, function()
			if not Get("SpamJump") then return end
			Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
		end))
		return "SpamJump", "Spam jump enabled"
	end,
})

Command.Add({
	Aliases = { "unspamjump" },
	Description = "Disable spam jump",
	Arguments = {},
	Task = function()
		Add("SpamJump", false)
		return "SpamJump", "Spam jump disabled"
...
Command.Add({
	...
	Task = function(Players)
		...
		return "TP", "Teleported"
	end,
})

Command.Add({
	Aliases = { "toggletool", "ttool" },
	Description = "Equip/unequip a tool by name",
	Arguments = {
		{ Name = "Tool", Type = "String" },
	},
	Task = function(ToolName)
		if not ToolName then return "ToggleTool", "Provide a tool name" end
		for _, Tool in next, Backpack:GetChildren() do
			if Lower(Tool.Name) == Lower(ToolName) then
				Tool.Parent = Character
				return "ToggleTool", Format("Equipped: %s", Tool.Name)
			end
		end
		for _, Tool in next, Character:GetChildren() do
			if Tool:IsA("Tool") and Lower(Tool.Name) == Lower(ToolName) then
				Tool.Parent = Backpack
				return "ToggleTool", Format("Unequipped: %s", Tool.Name)
			end
		end
		return "ToggleTool", Format("Tool '%s' not found", ToolName)
	end,
})

Command.Add({
	Aliases = { "equiptool", "equip" },
	Description = "Equip a tool by name",
	Arguments = {
		{ Name = "Tool", Type = "String" },
	},
	Task = function(ToolName)
		if not ToolName then return "Equip", "Provide a tool name" end
		local Target = ToolName == "*" and Backpack:GetChildren() or
			{ Backpack:FindFirstChild(ToolName) }
		local Count = 0
		for _, T in next, Target do
			if T and T:IsA("Tool") then
				T.Parent = Character
				Count += 1
			end
		end
		return "Equip", Format("Equipped %d tool(s)", Count)
	end,
})

Command.Add({
	Aliases = { "unequiptools2", "dequip" },
	Description = "Unequip all tools",
	Arguments = {},
	Task = function()
		for _, T in next, Character:GetChildren() do
			if T:IsA("Tool") then T.Parent = Backpack end
		end
		return "Unequip", "All tools unequipped"
	end,
})

Command.Add({
	Aliases = { "itemesp", "toolsesp" },
	Description = "Show ESP on tools in workspace",
	Arguments = {},
	Task = function()
		if Get("ItemESP") then
			Add("ItemESP", false)
			Cleaner:Clean("ItemESP")
			for _, V in next, workspace:GetDescendants() do
				if V:IsA("SelectionBox") and V.Name == "CMD_ItemESP" then V:Destroy() end
			end
			return "ItemESP", "Item ESP disabled"
		end
		Add("ItemESP", true)
		Cleaner:Add("ItemESP", Connect(Services.Run.Heartbeat, function()
			if not Get("ItemESP") then return end
			for _, V in next, workspace:GetDescendants() do
				if (V:IsA("Tool") or V:IsA("BackpackItem")) and not V:FindFirstChild("CMD_ItemESP") then
					local Sel = Instance.new("SelectionBox")
					Sel.Name = "CMD_ItemESP"
					Sel.Color3 = Color3.fromRGB(0, 255, 0)
					Sel.LineThickness = 0.05
					Sel.Adornee = V:FindFirstChild("Handle") or V
					Sel.Parent = V
				end
			end
		end))
		return "ItemESP", "Item ESP enabled"
	end,
})

Command.Add({
	Aliases = { "unitesp", "untoolsesp" },
	Description = "Disable item ESP",
	Arguments = {},
	Task = function()
		Add("ItemESP", false)
		Cleaner:Clean("ItemESP")
...
Command.Add({
	Aliases = { "itemtracers", "tooltracers" },
	Description = "Draw tracers to tools in workspace",
	Arguments = {},
	Task = function()
		if Get("ItemTracers") then
			Add("ItemTracers", false)
			local G = Get("ItemTracerGui")
			if G then G:Destroy(); Add("ItemTracerGui", nil) end
			return "ItemTracers", "Item tracers disabled"
		end
		local Gui = Instance.new("ScreenGui")
		Gui.Name = "CMD_ItemTracerGui"
		Gui.ResetOnSpawn = false
		Gui.IgnoreGuiInset = true
		Gui.Parent = game:GetService("CoreGui")
		Add("ItemTracerGui", Gui)
		Add("ItemTracers", true)
		Cleaner:Add("ItemTracers", Connect(Services.Run.RenderStepped, function()
			if not Get("ItemTracers") then return end
			for _, F in next, Gui:GetChildren() do F:Destroy() end
			local Cam = workspace.CurrentCamera
			for _, V in next, workspace:GetDescendants() do
				if V:IsA("Tool") or V:IsA("BackpackItem") then
					local H = V:FindFirstChild("Handle")
					if H then
						local TP, OS = Cam:WorldToViewportPoint(H.Position)
						if OS then
							local Line = Instance.new("Frame")
							Line.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
							Line.BorderSizePixel = 0
							local VX, VY = Cam.ViewportSize.X / 2, Cam.ViewportSize.Y
							local DX, DY = TP.X - VX, TP.Y - VY
							local Dist = math.sqrt(DX * DX + DY * DY)
							Line.Size = UDim2.new(0, 1, 0, Dist)
							Line.Position = UDim2.new(0, VX, 0, VY)
							Line.Rotation = math.deg(math.atan2(DX, -DY))
							Line.AnchorPoint = Vector2.new(0.5, 0)
							Line.Parent = Gui
						end
		...
		return "ItemTracers", "Item tracers enabled"
	end,
})

Command.Add({
	Aliases = { "unitemtracers" },
	Description = "Disable item tracers",
	Arguments = {},
	Task = function()
		Add("ItemTracers", false)
		local G = Get("ItemTracerGui")
...
Command.Add({
	Aliases = { "bringunanchored", "bua" },
	Description = "Bring all unanchored parts near you",
	Arguments = {
		{ Name = "Range", Type = "Number" },
	},
	Task = function(Range)
		...
		for _, V in next, workspace:GetDescendants() do
			if V:IsA("BasePart") and not V.Anchored
				and (Root.Position - V.Position).Magnitude <= Range
				and not Services.Players:GetPlayerFromCharacter(V.Parent) then
				V.CFrame = Root.CFrame * CFrame.new(
					math.random(-3, 3), math.random(-1, 1), math.random(-3, 3)
				)
				Count += 1
			end
		end
		return "BringUA", Format("Brought %d unanchored parts", Count)
	end,
})

Command.Add({
	Aliases = { "disabletouchevent", "dte" },
	Description = "Disable touch events on a part",
	Arguments = {},
	Task = function()
		local Target = Mouse.Target
		if not Target then return "DTE", "No target" end
		Target.TouchEnded:Connect(function() end)
		return "DTE", Format("Touch events disabled on %s", Target.Name)
	end,
})

Command.Add({
	Aliases = { "enabletouchevent", "ete" },
	Description = "Enable touch events on a part",
	Arguments = {},
	Task = function()
		local Target = Mouse.Target
		if not Target then return "ETE", "No target" end
		return "ETE", Format("Touch events noted on %s", Target.Name)
	end,
})

Command.Add({
	Aliases = { "creatorid", "cid2" },
	Description = "Show game creator ID",
	Arguments = {},
	Task = function()
		return "CreatorID", Format("Creator: %s (ID: %s)", game.CreatorType.Name, game.CreatorId)
	end,
})
...
Command.Add({
	...
	Task = function()
		...
		return "ServerInfo", Format("Players: %d/%d | PlaceID: %s",
			#Players,
